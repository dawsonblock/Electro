# ELECTRO v2.1.0 Performance Benchmark Report
...
- ELECTRO-only init (SQLite + tools + config): **~4 ms** (visible in logs)

### Comparison

| Runtime | Cold Start | Gateway Ready |
|---------|-----------|---------------|
| **ELECTRO** | **31 ms** | **1.4 s** (MCP-bound) |
...
- API metrics: ELECTRO's built-in `BudgetTracker` (logged per-call)
- Conversation: Automated 3-turn script piped via stdin to `electro chat`
...
1. **ELECTRO runs on hardware where OpenClaw cannot.** A 512 MB VPS can comfortably run ELECTRO with room to spare. OpenClaw requires at minimum 1.5 GB.

2. **Startup is effectively instant.** The 31ms cold start means ELECTRO can be used as a CLI tool invoked per-command, not just a long-running daemon. OpenClaw's 8-second startup forces daemon-only usage.
