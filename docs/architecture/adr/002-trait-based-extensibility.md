...
## Decision
Define 12 core traits in `electro-core`. Every subsystem is a trait implementation. Trait objects (`Box<dyn Trait>`) are used for runtime polymorphism where config determines the implementation.
