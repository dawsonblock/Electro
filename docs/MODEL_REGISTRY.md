# ELECTRO Model Registry

**Last verified: 2026-03-16** — All values sourced from official provider documentation and OpenRouter.

This registry defines context window and max output token limits for every known model. ELECTRO uses these values to set `max_context_tokens` and `max_output_tokens` automatically per model, so users get the full capability of their chosen model without manual tuning.

## How It Works

1. When a model is selected, ELECTRO looks up its limits in `model_limits()`
...
These models are commonly used through OpenRouter and other OpenAI-compatible proxies. ELECTRO recognizes them by name regardless of which proxy routes them.
