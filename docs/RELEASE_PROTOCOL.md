...
| Line ~425 | `electro update` example version | Match Cargo.toml version |
...
- **`crates/electro-skills/src/lib.rs`** — Test fixtures use hardcoded version strings. These are test data, not release metadata.
...
| Mistake | Consequence |
|---------|-------------|
| Bump README but not Cargo.toml | `electro -V` shows old version |
| Bump Cargo.toml but not README badges | GitHub page shows old version |
| Forget `electro update` example version | Users see wrong version in help output |
