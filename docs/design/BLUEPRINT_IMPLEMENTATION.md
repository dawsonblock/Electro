...
This document specifies **exactly** how to implement the Blueprint system in the ELECTRO codebase. Every file touched, every struct added, every function written — with risk analysis for each change.
...
**File**: `crates/electro-core/src/traits/memory.rs`
...
**File**: `crates/electro-agent/src/blueprint.rs` (NEW FILE)
...
pub async fn find_matching_blueprint(
    memory: &dyn electro_core::Memory,
    task_text: &str,
) -> Option<Blueprint> {
    use electro_core::{SearchOpts, MemoryEntryType};
...
pub fn to_memory_entry(
    ...
    electro_core::MemoryEntry {
        ...
        session_id,
        entry_type: electro_core::MemoryEntryType::Blueprint,
    }
}
```
...
**File**: `crates/electro-agent/src/lib.rs`
...
**File**: `crates/electro-agent/Cargo.toml`

**Change**: Add `serde_yaml` to dependencies (it's already a transitive dependency via `electro-skills`, so this adds no new code to the binary — just makes it a direct dependency for blueprint parsing).
...
**Verification**: Check that `electro-skills` already uses `serde_yaml` — if so, Cargo deduplicates automatically.
...
async fn author_blueprint(
    ...
    let text = response.text.ok_or_else(||
        ElectroError::Provider("Blueprint authoring returned no text".into())
    )?;

    let mut bp = crate::blueprint::parse_blueprint(&text)
        .map_err(|e| ElectroError::Provider(format!("Failed to parse authored blueprint: {e}")))?;
...
async fn refine_blueprint(
    ...
    let response = provider.complete(request).await?;
    let text = response.text.ok_or_else(||
        ElectroError::Provider("Blueprint refinement returned no text".into())
    )?;

    let refined = crate::blueprint::parse_blueprint(&text)
        .map_err(|e| ElectroError::Provider(format!("Failed to parse refined blueprint: {e}")))?;
...
**File**: `crates/electro-agent/src/runtime.rs` (the `build_context` call in the tool loop)
...
**File**: `crates/electro-agent/src/context.rs` (tests)
...
|---|---|---|
| `crates/electro-core/src/traits/memory.rs` | Add `Blueprint` variant to enum | ZERO — additive, backwards-compatible |
| `crates/electro-agent/src/blueprint.rs` | **NEW FILE** — all blueprint logic | ZERO — new code, nothing touched |
| `crates/electro-agent/src/lib.rs` | Add `pub mod blueprint` + `pub use` | ZERO — additive |
| `crates/electro-agent/Cargo.toml` | Add `serde_yaml` dep | ZERO — already transitive |
| `crates/electro-agent/src/runtime.rs` | Add blueprint matching (pre-loop) + authoring (post-DONE) | ZERO — appended code, existing code untouched |
| `crates/electro-agent/src/context.rs` | Add `active_blueprint` param + injection block | LOW — signature change requires call-site updates, but all pass `None` |
| `crates/electro-agent/src/context.rs` (tests) | Add `None` to `build_context` calls | ZERO — identical behavior |
...
2. **Blueprint CLI commands**: `electro blueprints list`, `electro blueprints show <id>`, `electro blueprints delete <id>` — user management (add in follow-up)
3. **Filesystem cache**: `~/.electro/blueprints/*.md` — human-readable copies (add in follow-up)
