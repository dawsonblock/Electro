...
### Step 1: Config flag (electro-core)

Add `parallel_phases: bool` to `AgentConfig` with `default = false`.

**Files:** `crates/electro-core/src/types/config.rs`
**Risk:** Zero — additive field with serde default

### Step 2: Phase parser (electro-agent/blueprint.rs)
...
**Files:** `crates/electro-agent/src/blueprint.rs`
**Risk:** Zero — pure function, no side effects, doesn't change existing code

### Step 3: Phase-to-TaskGraph bridge (electro-agent/blueprint.rs)

Add `phases_to_task_graph(phases: Vec<BlueprintPhase>, goal: &str) -> Result<TaskGraph, ElectroError>`.
...
### Step 4: Phase executor (electro-agent/runtime.rs)
...
**Files:** `crates/electro-agent/src/runtime.rs`
**Risk:** Low — gated behind opt-in flag, falls through to existing code when off

### Step 5: Authoring prompt update (electro-agent/blueprint.rs)
...
### Step 6: Wire flag through runtime (electro-agent + main.rs)
...
**Files:** `crates/electro-agent/src/runtime.rs`, `src/main.rs`
...
**Files:** `crates/electro-agent/src/blueprint.rs`, test modules
...
|------|--------|-------------|
| `crates/electro-core/src/types/config.rs` | Add `parallel_phases` field | +5 |
| `crates/electro-agent/src/blueprint.rs` | Add phase parser + bridge | +150 |
| `crates/electro-agent/src/runtime.rs` | Add phase executor + flag | +120 |
