...
ELECTRO supports three deployment methods: Docker, Fly.io, and Terraform (AWS). All methods use the same static binary and configuration format.
...
```bash
docker pull ghcr.io/electro/electro:latest

docker run -d \
  --name electro \
  --restart unless-stopped \
  -p 8080:8080 \
  -v electro-data:/var/lib/electro \
  -e ELECTRO_MODE=cloud \
  -e ANTHROPIC_API_KEY=sk-ant-... \
  -e TELEGRAM_BOT_TOKEN=123456:ABC-... \
  -e RUST_LOG=info \
  ghcr.io/electro/electro:latest
```
...
docker build -t electro:latest .

# Build for a specific platform
docker buildx build --platform linux/amd64 -t electro:latest .
docker buildx build --platform linux/arm64 -t electro:latest .
```
...
| User | `electro` (non-root) |
| Data directory | `/var/lib/electro` |
| Config directory | `/etc/electro` |
| Exposed port | `8080` |
| Health check | `GET http://localhost:8080/health` (30s interval) |
| Entry point | `electro start` |
...
services:
  electro:
    ...
    volumes:
      ...
      - ./config.toml:/etc/electro/config.toml:ro
    environment:
      ELECTRO_MODE: cloud
...
The `/var/lib/electro` directory contains:
...
Mount a config file to `/etc/electro/config.toml`:

```bash
docker run -d \
  -v ./my-config.toml:/etc/electro/config.toml:ro \
  -v electro-data:/var/lib/electro \
  ghcr.io/electro/electro:latest
```
...
| Persistent volume | `electro_data` mounted at `/var/lib/electro` (1 GB initial) |
...
The Terraform configuration deploys ELECTRO on a single EC2 instance with persistent EBS storage.
...
| `docker_image` | `ghcr.io/electro/electro` | Docker image |
| `docker_tag` | `latest` | Docker image tag |
| `electro_mode` | `auto` | ELECTRO operating mode |
...
- `gateway_url` -- ELECTRO gateway URL (http://IP:8080)
...
2. Formats and mounts the EBS data volume to `/var/lib/electro`
3. Pulls the ELECTRO Docker image
...
This removes all AWS resources. The EBS volume is destroyed -- back up `/var/lib/electro` first if needed.
