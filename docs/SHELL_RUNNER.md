...
TEMM1E now prefers an isolated container runner for shell execution.

## Default behavior

- `TEMM1E_SHELL_BACKEND=auto` tries `docker` first, then `podman`.
- The runner mounts only the session workspace at `/workspace`.
- The root filesystem is read-only.
- `/tmp` is a tmpfs.
- Linux capabilities are dropped.
- Network is disabled unless `TEMM1E_SHELL_ALLOW_NETWORK=1`.
...
```bash
./scripts/build_shell_runner.sh temm1e-shell-runner:local
export TEMM1E_SHELL_CONTAINER_IMAGE=temm1e-shell-runner:local
```

## Smoke test the runner

```bash
./scripts/smoke_shell_runner.sh temm1e-shell-runner:local
```
...
```bash
export TEMM1E_SHELL_BACKEND=host
export TEMM1E_ENABLE_HOST_SHELL=1
```

Additional notes:

- Host fallback no longer inherits the full parent environment.
- `HOME`, `XDG_CACHE_HOME`, `XDG_CONFIG_HOME`, `XDG_STATE_HOME`, and `TMPDIR` are redirected into workspace-local `.temm1e-host-*` directories.
- Shell launchers such as `sh`, `bash`, `dash`, `zsh`, and `sudo` are blocked in host fallback.
- If you really need to pass through selected secrets or tokens, set `TEMM1E_SHELL_PASSTHROUGH_ENV` to a comma-separated allowlist of exact environment variable names.
