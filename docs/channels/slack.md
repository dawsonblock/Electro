...
ELECTRO polls Slack for new messages (no webhook server needed). Send a message in any channel the bot is invited to, and it will respond.
...
The allowlist persists at `~/.electro/slack_allowlist.toml`.
...
Slack has a 4000-character message limit. ELECTRO automatically splits long responses at natural boundaries (newlines, then spaces).

## How It Works

ELECTRO uses **polling** (not Socket Mode or webhooks):
...
| Rate limiting | ELECTRO includes 100ms delays between API calls; Slack free-tier has stricter limits |
