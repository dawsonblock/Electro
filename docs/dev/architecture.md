...
This document describes ELECTRO's internal architecture: the crate dependency graph, data flow through the system, async runtime model, and extension points for adding new functionality.

## Design Principles

1. **Trait-based extensibility** -- every subsystem is defined by a trait in `electro-core`; implementations live in separate crates
...
**Key rule**: all crates depend on `electro-core` for trait definitions and shared types. No implementation crate depends on another implementation crate. This keeps the dependency graph clean and enables independent compilation.

## Core Crate: electro-core

The `electro-core` crate contains:
...
- **Shared types** (`types/`) -- `InboundMessage`, `OutboundMessage`, `CompletionRequest`, `CompletionResponse`, `ElectroError`, `ElectroConfig`, etc.
- **Config loading** (`config/`) -- TOML parser, YAML compat reader, environment variable expansion, `vault://` URI resolution

`electro-core` has minimal external dependencies: `serde`, `async-trait`, `thiserror`, `chrono`, `bytes`, `futures`. It defines interfaces only; no business logic.
...
ELECTRO uses **Tokio** as its async runtime with the multi-threaded scheduler.
...
| Layer | Approach |
|-------|----------|
| `electro-core` types | `ElectroError` enum with `thiserror` -- domain-specific variants |
| Implementation crates | Return `Result<T, ElectroError>` at crate boundaries |
...
```
sqlx::Error --> ElectroError::Memory("...") --> anyhow::Error (at binary level)
...
Compiled defaults
  |
  v
/etc/electro/config.toml (system)
  |
  v
~/.electro/config.toml (user)
  |
  v
./config.toml (workspace)
  |
  v
ELECTRO_* environment variables
  |
  v
...
Later sources override earlier ones. The config module in `electro-core/src/config/` handles:
- `loader.rs` -- file discovery and loading
- `toml.rs` -- native TOML parser
- `yaml_compat.rs` -- OpenClaw YAML format reader
- `env.rs` -- `${ENV_VAR}` expansion and `ELECTRO_*` mapping
