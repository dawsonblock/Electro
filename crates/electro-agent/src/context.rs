...
use electro_core::types::optimization::PromptTier;
use electro_core::types::session::SessionContext;
use electro_core::MemoryEntryType;
use electro_core::{Memory, SearchOpts, Tool};
...
pub async fn build_context(
    ...
    let system = match prompt_tier {
        Some(tier) if system_prompt.is_none() => {
            ...
            let config = electro_core::types::config::AgentConfig::default();
...

    {
        ...
        let lambda_config = electro_core::types::config::LambdaMemoryConfig {
...
fn build_system_prompt(
    ...
    custom.map(|s| s.to_string()).or_else(|| {
        ...
        Some(format!(
            "You are ELECTRO, a cloud-native AI agent runtime. You control a computer through messaging apps.\n\
             \n\
...
mod tests {
    use super::*;
    use electro_test_utils::{make_session, MockMemory, MockTool};
    ...
    async fn context_default_system_prompt() {
        ...
        assert!(req.system.unwrap().contains("ELECTRO"));
    }
