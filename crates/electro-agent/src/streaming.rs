...
impl StreamingNotifier {
    ...
    pub async fn notify_tool_start(&self, tool_name: &str) -> Result<(), ElectroError> {
    ...
    pub async fn notify_status(&self, message: &str) -> Result<(), ElectroError> {
        if !self.enabled {
            return Ok(());
        }
    ...
    async fn send_status(&self, text: &str) -> Result<(), ElectroError> {
    ...
    impl Channel for MockChannel {
        ...
        async fn send_message(&self, msg: OutboundMessage) -> Result<(), ElectroError> {
            if let Some(ref err) = self.fail_with {
                return Err(ElectroError::Channel(err.clone()));
            }
