...
fn build_classify_prompt(available_categories: &[String], mode: ElectroMode) -> String {
    let mut prompt = CLASSIFY_BASE_PROMPT.to_string();

    // Inject personality mode
    prompt.push_str(match mode {
        ElectroMode::Play => CLASSIFY_MODE_PLAY,
        ElectroMode::Work => CLASSIFY_MODE_WORK,
        ElectroMode::Pro => CLASSIFY_MODE_PRO,
        ElectroMode::None => CLASSIFY_MODE_NONE,
    });
...
pub async fn classify_message(
    ...
    mode: ElectroMode,
) -> Result<(MessageClassification, Usage), ElectroError> {
...
fn parse_classification(text: &str) -> Result<MessageClassification, ElectroError> {
    ...
    serde_json::from_str::<MessageClassification>(json_str).map_err(|e| {
        ...
        ElectroError::Provider(format!("Classification parse error: {}", e))
    })
}
...
mod tests {
    ...
    fn build_prompt_without_categories() {
        let prompt = build_classify_prompt(&[], ElectroMode::Play);
    ...
    fn build_prompt_with_categories() {
        ...
        let prompt = build_classify_prompt(&categories, ElectroMode::Play);
        assert!(prompt.contains("blueprint_hint"));
