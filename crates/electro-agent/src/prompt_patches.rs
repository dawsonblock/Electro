...
impl PromptPatchManager {
    ...
    pub fn propose_patch(&mut self, mut patch: PromptPatch) -> Result<String, ElectroError> {
    ...
    pub fn approve_patch(&mut self, id: &str) -> Result<(), ElectroError> {
        let patch = self
            .patches
            .iter_mut()
            .find(|p| p.id == id)
            .ok_or_else(|| ElectroError::NotFound(format!("Prompt patch not found: {id}")))?;
    ...
    pub fn reject_patch(&mut self, id: &str) -> Result<(), ElectroError> {
        let patch = self
            .patches
            .iter_mut()
            .find(|p| p.id == id)
