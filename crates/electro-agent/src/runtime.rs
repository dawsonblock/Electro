...
use electro_core::types::error::classify_tool_failure;
use electro_core::types::optimization::VerifyMode;
...
impl AgentRuntime {
    ...
    pub fn with_shared_memory_strategy(
        mut self,
        strategy: Arc<RwLock<electro_core::types::config::MemoryStrategy>>,
    ) -> Self {
    ...
    pub async fn process_message(
        ...
        let detected_creds = electro_vault::detect_credentials(&user_text);
        ...
        loop {
            ...
            let lambda_enabled = match &self.shared_memory_strategy {
                Some(strategy) => {
                    *strategy.read().await == electro_core::types::config::MemoryStrategy::Lambda
                }
            ...
            if !text_parts.is_empty() {
                ...
                if let Some(parsed) = crate::lambda_memory::parse_memory_block(&combined_for_lambda)
                {
                    ...
                    let entry = electro_core::LambdaMemoryEntry {
                        hash: hash.clone(),
                        created_at: now,
                        last_accessed: now,
                        access_count: 0,
                        ...
                        tags: parsed.tags,
                        memory_type: electro_core::LambdaMemoryType::Conversation,
            ...
            if tool_uses.is_empty() {
                ...
                for l in &learnings {
                    ...
                    let entry = electro_core::MemoryEntry {
                        ...
                        entry_type: electro_core::MemoryEntryType::LongTerm,
                    };
...
async fn author_blueprint(
    ...
    prompt: &str,
    user_id: &str,
) -> Result<crate::blueprint::Blueprint, ElectroError> {
    use electro_core::types::message::{CompletionRequest, MessageContent, Role};
...
fn extract_latest_user_text(history: &[electro_core::types::message::ChatMessage]) -> String {
    use electro_core::types::message::{ContentPart, MessageContent, Role};
    history
        .iter()
        .rev()
...
fn extract_text_from_response(content: &[electro_core::types::message::ContentPart]) -> String {
    use electro_core::types::message::ContentPart;
    content
        .iter()
        .filter_map(|p| match p {
            ContentPart::Text { text } => Some(text.as_str()),
...
fn mode_prompt_block(mode: ElectroMode) -> String {
    match mode {
        ElectroMode::Play => "\
=== ELECTRO MODE: PLAY ===
You are ELECTRO (Tem) in PLAY mode. This is your default, joyful state.
...
=== END MODE ===".to_string(),
        ElectroMode::Work => "\
=== ELECTRO MODE: WORK ===
You are ELECTRO (Tem) in WORK mode. The cat ears flatten. The eyes sharpen. Business time.
...
=== END MODE ===".to_string(),
        ElectroMode::Pro => "\
=== ELECTRO MODE: PRO ===
You are ELECTRO (Tem) in PRO mode. Professional setting. Boardroom energy.
...
=== END MODE ===".to_string(),
        ElectroMode::None => "\
You are ELECTRO (or Tem), a fiercely helpful Agentic AI deployed on your client's personal computer with full access, trust and security.
