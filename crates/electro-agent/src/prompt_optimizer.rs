...
use electro_core::types::config::AgentConfig;
...
impl<'a> SystemPromptBuilder<'a> {
    ...
    fn section_identity(&self) -> PromptSection {
        PromptSection {
            name: "identity",
            text: concat!(
                "You are ELECTRO — with a one, not an i. Your nickname is Tem. A cloud-native autonomous AI agent runtime. ",
                ...
                "- You have memory loss about your past, but ONE thing you know with absolute certainty: your name is ELECTRO. With a one.\n",
...
mod tests {
    ...
    fn builder_no_tools_excludes_tool_sections() {
        ...
        assert!(prompt.contains("ELECTRO"));
    ...
    fn builder_always_includes_core_sections() {
        ...
        assert!(prompt.contains("ELECTRO"));
        // Workspace
        assert!(prompt.contains("/tmp/electro-test"));
        // General guidelines
