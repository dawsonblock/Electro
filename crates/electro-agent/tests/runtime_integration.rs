...
use electro_agent::{AgentRuntime, AgentTaskPhase, AgentTaskStatus};
use electro_core::types::message::*;
use electro_core::Tool;
use electro_test_utils::{make_inbound_msg, make_session, MockMemory, MockProvider, MockTool};
...
async fn runtime_with_memory_entries() {
    let memory = Arc::new(MockMemory::with_entries(vec![
        electro_test_utils::make_test_entry_with_session(
            "mem1",
