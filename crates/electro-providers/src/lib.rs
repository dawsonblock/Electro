...
use electro_core::types::config::ProviderConfig;
...
pub fn create_provider(config: &ProviderConfig) -> Result<Box<dyn Provider>, ElectroError> {
    ...
    let api_key = all_keys
        ...
        .ok_or_else(|| ElectroError::Config("Provider api_key is required".into()))?;
