...
fn convert_message_to_anthropic(msg: &ChatMessage) -> Result<serde_json::Value, ElectroError> {
...
impl Provider for AnthropicProvider {
    ...
    async fn complete(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| ElectroError::Provider(format!("Anthropic request failed: {e}")))?;
        ...
        let api_response: AnthropicResponse = response.json().await.map_err(|e| {
            ElectroError::Provider(format!("Failed to parse Anthropic response: {e}"))
        })?;
    ...
    async fn stream(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| ElectroError::Provider(format!("Anthropic stream request failed: {e}")))?;
