            ...
            if matches!(msg.role, Role::Assistant) {
                ...
                if tool_messages.len() == 1 {
                    return tool_messages.into_iter().next().ok_or_else(|| {
                        ElectroError::Provider("Expected tool message but got none".into())
                    });
                }
...
impl Provider for OpenAICompatProvider {
    ...
    async fn complete(
        ...
        let body_text = response.text().await.map_err(|e| {
            ElectroError::Provider(format!("Failed to read OpenAI-compat response body: {e}"))
        })?;
        ...
        let choice = api_response
            ...
            .into_iter()
            .next()
            .ok_or_else(|| ElectroError::Provider("No choices in response".into()))?;
    ...
    async fn stream(
        ...
        let response = req.json(&body).send().await.map_err(|e| {
            ElectroError::Provider(format!("OpenAI-compat stream request failed: {e}"))
        })?;
    ...
    async fn list_models(&self) -> Result<Vec<String>, ElectroError> {
        ...
        let resp = req
            .send()
            .await
            .map_err(|e| ElectroError::Provider(format!("Failed to list models: {e}")))?;
        ...
        let models_response: OpenAIModelsResponse = resp
            .json()
            .await
            .map_err(|e| ElectroError::Provider(format!("Failed to parse models response: {e}")))?;
...
) -> Option<Result<StreamChunk, ElectroError>> {
    loop {
        ...
        for choice in &chunk.choices {
            ...
            if choice.finish_reason.as_deref() == Some("error") {
                ...
                return Some(Err(ElectroError::Provider(format!(
...
mod tests {
    ...
    fn with_extra_headers() {
        ...
        headers.insert("X-Title".to_string(), "ELECTRO".to_string());
