...
impl HttpTransport {
    pub fn new(
        ...
        timeout: Duration,
        extra_headers: HashMap<String, String>,
    ) -> Result<Self, ElectroError> {
        let client = reqwest::Client::builder()
            .timeout(timeout)
            .build()
            .map_err(|e| ElectroError::Tool(format!("Failed to create HTTP client: {}", e)))?;
    ...
    fn build_request(&self, body: &str) -> Result<reqwest::RequestBuilder, ElectroError> {
...
impl Transport for HttpTransport {
    async fn send(
        ...
        let json = serde_json::to_string(&request)
            .map_err(|e| ElectroError::Tool(format!("Failed to serialize request: {}", e)))?;
        ...
        let body = http_resp.text().await.map_err(|e| {
            ElectroError::Tool(format!("Failed to read MCP HTTP response body: {}", e))
        })?;
    ...
    async fn notify(
        ...
        let json = serde_json::to_string(&notification)
            .map_err(|e| ElectroError::Tool(format!("Failed to serialize notification: {}", e)))?;
