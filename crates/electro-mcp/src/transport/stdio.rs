...
impl StdioTransport {
    ...
    pub async fn spawn(config: &McpServerConfig, timeout: Duration) -> Result<Self, ElectroError> {
...
impl Transport for StdioTransport {
    async fn send(
        ...
        let json = serde_json::to_string(&request).map_err(|e| {
            ElectroError::Tool(format!("Failed to serialize JSON-RPC request: {}", e))
        })?;
    ...
    async fn notify(
        ...
        stdin
            .write_all(b"\n")
            .await
            .map_err(|e| ElectroError::Tool(format!("Failed to write newline: {}", e)))?;
        stdin
            .flush()
            .await
            .map_err(|e| ElectroError::Tool(format!("Failed to flush: {}", e)))?;
        Ok(())
    }
