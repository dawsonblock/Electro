//! McpBridgeTool — adapter that wraps an MCP server's tool as a ELECTRO Tool.
//!
//! The agent sees MCP tools identically to built-in tools. The bridge
//! translates between ELECTRO's Tool trait and MCP's JSON-RPC protocol.
...
/// Bridge adapter: wraps a single MCP tool as a ELECTRO Tool.
