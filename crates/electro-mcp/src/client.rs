...
impl McpClient {
    ...
    pub async fn initialize(&self) -> Result<McpServerInfo, ElectroError> {
    ...
    pub async fn list_tools(&self) -> Result<Vec<McpToolInfo>, ElectroError> {
        let response = self.transport.send("tools/list", None).await?;
        let result = response.into_result()?;

        let tools_array = result["tools"]
            .as_array()
            .ok_or_else(|| ElectroError::Tool("MCP tools/list: missing 'tools' array".into()))?;
    ...
    ) -> Result<McpToolResult, ElectroError> {
        let params = serde_json::json!({
            "name": tool_name,
            "arguments": arguments
        });
    ...
    pub async fn ping(&self) -> Result<(), ElectroError> {
    ...
    pub async fn close(&self) -> Result<(), ElectroError> {
