...
impl McpManager {
    /// Create a new McpManager and load config from `~/.electro/mcp.toml`.
    pub fn new() -> Self {
    ...
    async fn connect_server(&self, config: McpServerConfig) -> Result<usize, ElectroError> {
        ...
        let transport: Arc<dyn Transport> = match config.transport.as_str() {
            ...
            "http" => Arc::new(HttpTransport::new(
                ...
                config
                    .url
                    .as_deref()
                    .ok_or_else(|| ElectroError::Tool("HTTP transport requires a URL".into()))?,
                timeout,
                config.headers.clone(),
            )?),
            other => {
                return Err(ElectroError::Tool(format!("Unknown transport: {}", other)));
            }
        };
    ...
    pub async fn add_server(&self, config: McpServerConfig) -> Result<usize, ElectroError> {
        config.validate().map_err(ElectroError::Tool)?;
    ...
    pub async fn remove_server(&self, name: &str) -> Result<(), ElectroError> {
        let mut servers = self.servers.write().await;
    ...
    pub async fn restart_server(&self, name: &str) -> Result<usize, ElectroError> {
        let config = {
            let servers = self.servers.read().await;
