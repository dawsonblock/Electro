...
impl SqliteUsageStore {
    ...
    pub async fn from_pool(pool: SqlitePool) -> Result<Self, ElectroError> {
        let store = Self { pool };
    ...
    async fn init_tables(&self) -> Result<(), ElectroError> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Memory(format!("Failed to create usage_log table: {e}")))?;
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Memory(format!("Failed to create usage index: {e}")))?;
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Memory(format!("Failed to create usage_display table: {e}")))?;
...
impl UsageStore for SqliteUsageStore {
    async fn record_usage(&self, record: UsageRecord) -> Result<(), ElectroError> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Memory(format!("Failed to insert usage record: {e}")))?;
        Ok(())
    }
    ...
    async fn query_usage(
        ...
        chat_id: &str,
        limit: Option<u32>,
    ) -> Result<Vec<UsageRecord>, ElectroError> {
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| ElectroError::Memory(format!("Failed to query usage: {e}")))?;

        rows.into_iter().map(|r| r.into_record()).collect()
    }

    async fn usage_summary(&self, chat_id: &str) -> Result<UsageSummary, ElectroError> {
        ...
        .fetch_one(&self.pool)
        .await
        .map_err(|e| ElectroError::Memory(format!("Failed to get usage summary: {e}")))?;
    ...
    async fn set_usage_display(&self, chat_id: &str, enabled: bool) -> Result<(), ElectroError> {
        sqlx::query(
            r#"
    ...
    async fn is_usage_display_enabled(&self, chat_id: &str) -> Result<bool, ElectroError> {
        let row: Option<(i64,)> =
            sqlx::query_as("SELECT enabled FROM usage_display WHERE chat_id = ?")
                ...
                .await
                .map_err(|e| {
                    ElectroError::Memory(format!("Failed to query usage display preference: {e}"))
                })?;
...
impl UsageRow {
    fn into_record(self) -> Result<UsageRecord, ElectroError> {
