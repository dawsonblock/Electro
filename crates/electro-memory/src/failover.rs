...
impl ResilientMemory {
    ...
    pub async fn attempt_repair(&self) -> Result<(), ElectroError> {
        info!(
        ...
        match self.primary.list_sessions().await {
            ...
            Err(e) => {
                ...
                Err(ElectroError::Memory(format!(
    ...
    async fn record_failure(&self, err: &ElectroError) -> bool {
        let mut state = self.state.write().await;
        state.consecutive_failures += 1;
    ...
    async fn sync_cache_to_primary(&self) -> Result<(), ElectroError> {
...
mod tests {
    ...
    impl Memory for FakeMemory {
        async fn store(&self, entry: MemoryEntry) -> Result<(), ElectroError> {
            if self.failing() {
                return Err(ElectroError::Memory("fake store failure".into()));
            }
        ...
        async fn search(
            ...
            if self.failing() {
                return Err(ElectroError::Memory("fake search failure".into()));
            }
        ...
        async fn get(&self, id: &str) -> Result<Option<MemoryEntry>, ElectroError> {
            if self.failing() {
                return Err(ElectroError::Memory("fake get failure".into()));
            }
        ...
        async fn delete(&self, id: &str) -> Result<(), ElectroError> {
            if self.failing() {
                return Err(ElectroError::Memory("fake delete failure".into()));
            }
        ...
        async fn list_sessions(&self) -> Result<Vec<String>, ElectroError> {
            if self.failing() {
                return Err(ElectroError::Memory("fake list_sessions failure".into()));
            }
            let store = self.store.read().await;
        ...
        async fn get_session_history(
            ...
            if self.failing() {
                return Err(ElectroError::Memory(
