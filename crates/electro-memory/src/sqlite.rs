...
impl SqliteMemory {
    ...
    pub async fn new(database_url: &str) -> Result<Self, ElectroError> {
        let pool = SqlitePoolOptions::new()
            ...
            .await
            .map_err(|e| ElectroError::Memory(format!("Failed to connect to SQLite: {e}")))?;
    ...
    async fn init_tables(&self) -> Result<(), ElectroError> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Memory(format!("Failed to create lambda FTS5: {e}")))?;
...
impl Memory for SqliteMemory {
    async fn store(&self, entry: MemoryEntry) -> Result<(), ElectroError> {
        let metadata_str =
            serde_json::to_string(&entry.metadata).map_err(ElectroError::Serialization)?;
        ...
        timeout(DB_TIMEOUT, async {
            ...
            if let Some(e) = last_err {
                return Err(ElectroError::Memory(format!("Failed to store entry: {e}")));
            }
            Ok(())
        })
        .await
        .map_err(|_| {
            ElectroError::Memory("Database operation timed out after 5 seconds".into())
        })??;
    ...
    async fn get(&self, id: &str) -> Result<Option<MemoryEntry>, ElectroError> {
        ...
        .await
        .map_err(|_| ElectroError::Memory("Database operation timed out after 5 seconds".into()))?
        .map_err(|e| ElectroError::Memory(format!("Failed to get entry: {e}")))?;
    ...
    async fn delete(&self, id: &str) -> Result<(), ElectroError> {
        ...
        .await
        .map_err(|_| ElectroError::Memory("Database operation timed out after 5 seconds".into()))?
        .map_err(|e| ElectroError::Memory(format!("Failed to delete entry: {e}")))?;
    ...
    async fn list_sessions(&self) -> Result<Vec<String>, ElectroError> {
        ...
        .await
        .map_err(|_| ElectroError::Memory("Database operation timed out after 5 seconds".into()))?
        .map_err(|e| ElectroError::Memory(format!("Failed to list sessions: {e}")))?;
    ...
    async fn get_session_history(
        ...
        .await
        .map_err(|_| ElectroError::Memory("Database operation timed out after 5 seconds".into()))?
        .map_err(|e| ElectroError::Memory(format!("Failed to get session history: {e}")))?;
    ...
    async fn lambda_store(&self, entry: LambdaMemoryEntry) -> Result<(), ElectroError> {
        ...
        let rowid: Option<(i64,)> =
            sqlx::query_as("SELECT rowid FROM lambda_memories WHERE hash = ?")
                ...
                .fetch_optional(&self.pool)
                .await
                .map_err(|e| ElectroError::Memory(format!("lambda_store FTS rowid lookup: {e}")))?;
        ...
        if let Some((rid,)) = rowid {
            ...
            .execute(&self.pool)
            .await
            .map_err(|e| ElectroError::Memory(format!("lambda_store FTS insert: {e}")))?;
        }
    ...
    async fn lambda_query_candidates(
        &self,
        limit: usize,
    ) -> Result<Vec<LambdaMemoryEntry>, ElectroError> {
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| ElectroError::Memory(format!("lambda_query_candidates: {e}")))?;
    ...
    async fn lambda_recall(
        &self,
        hash_prefix: &str,
    ) -> Result<Option<LambdaMemoryEntry>, ElectroError> {
        ...
        .fetch_optional(&self.pool)
        .await
        .map_err(|e| ElectroError::Memory(format!("lambda_recall: {e}")))?;
    ...
    async fn lambda_touch(&self, hash: &str) -> Result<(), ElectroError> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Memory(format!("lambda_touch: {e}")))?;
    ...
    async fn lambda_fts_search(
        ...
        query: &str,
        limit: usize,
    ) -> Result<Vec<(String, f64)>, ElectroError> {
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| ElectroError::Memory(format!("lambda_fts_search: {e}")))?;
        ...
        for (rowid, rank) in rows {
            let hash_row: Option<(String,)> =
                sqlx::query_as("SELECT hash FROM lambda_memories WHERE rowid = ?")
                    ...
                    .fetch_optional(&self.pool)
                    .await
                    .map_err(|e| ElectroError::Memory(format!("lambda_fts hash resolve: {e}")))?;
    ...
    async fn lambda_gc(&self, now_epoch: u64, max_age_secs: u64) -> Result<usize, ElectroError> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Memory(format!("lambda_gc: {e}")))?;
...
fn row_to_entry(row: MemoryRow) -> Result<MemoryEntry, ElectroError> {
    let metadata: serde_json::Value =
        serde_json::from_str(&row.metadata).map_err(ElectroError::Serialization)?;
    let timestamp = chrono::DateTime::parse_from_rfc3339(&row.timestamp)
        .map_err(|e| ElectroError::Memory(format!("Invalid timestamp: {e}")))?
...
fn str_to_entry_type(s: &str) -> Result<MemoryEntryType, ElectroError> {
    match s {
        ...
        other => Err(ElectroError::Memory(format!("Unknown entry type: {other}"))),
    }
}
