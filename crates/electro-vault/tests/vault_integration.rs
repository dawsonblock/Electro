...
use electro_vault::local::LocalVault;
use electro_vault::{detect_credentials, is_vault_uri, parse_vault_uri, resolve};
...
async fn vault_store_and_resolve_uri() {
    ...
    let result = resolve(&vault, "vault://electro/api/key").await.unwrap();
...
async fn vault_resolver_invalid_uri_errors() {
    ...
    let result = resolve(&vault, "http://electro/key").await;
...
async fn vault_resolver_missing_key_returns_none() {
    ...
    let result = resolve(&vault, "vault://electro/nonexistent").await.unwrap();
...
fn credential_detection_and_vault_uri_integration() {
    ...
    let uri = format!("vault://electro/{}", creds[0].key);
