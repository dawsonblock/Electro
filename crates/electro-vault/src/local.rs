...
///
/// - Vault file: `~/.electro/vault.enc` (JSON map of key -> StoredSecret)
/// - Key file:   `~/.electro/vault.key` (32 raw bytes)
...
impl LocalVault {
    ...
    pub async fn with_dir(dir: PathBuf) -> Result<Self, ElectroError> {
        tokio::fs::create_dir_all(&dir)
            .await
            .map_err(|e| ElectroError::Vault(format!("failed to create vault directory: {e}")))?;
    ...
    async fn ensure_key(&self) -> Result<(), ElectroError> {
        ...
        tokio::fs::write(&self.key_path, key_bytes.as_ref())
            .await
            .map_err(|e| ElectroError::Vault(format!("failed to write vault key: {e}")))?;
    ...
    async fn read_key(&self) -> Result<Zeroizing<[u8; 32]>, ElectroError> {
        let bytes = tokio::fs::read(&self.key_path)
            .await
            .map_err(|e| ElectroError::Vault(format!("failed to read vault key: {e}")))?;

        let key: [u8; 32] = bytes
            .try_into()
            .map_err(|_| ElectroError::Vault("vault key must be exactly 32 bytes".into()))?;
    ...
    fn encrypt(
        ...
        plaintext: &[u8],
    ) -> Result<(Vec<u8>, [u8; 12]), ElectroError> {
        ...
        let ciphertext = cipher
            ...
            .map_err(|e| ElectroError::Vault(format!("encryption failed: {e}")))?;
    ...
    fn decrypt(
        ...
        nonce_bytes: &[u8; 12],
        ciphertext: &[u8],
    ) -> Result<Vec<u8>, ElectroError> {
        let nonce = Nonce::from_slice(nonce_bytes);
        cipher
            .decrypt(nonce, ciphertext)
            .map_err(|e| ElectroError::Vault(format!("decryption failed: {e}")))?
            .pipe_ok()
    }
    ...
    async fn load(&self) -> Result<(), ElectroError> {
        ...
        let data = tokio::fs::read_to_string(&self.vault_path)
            .await
            .map_err(|e| ElectroError::Vault(format!("failed to read vault file: {e}")))?;
        ...
        let map: HashMap<String, StoredSecret> = serde_json::from_str(&data)
            .map_err(|e| ElectroError::Vault(format!("corrupt vault file: {e}")))?;
    ...
    async fn flush(&self) -> Result<(), ElectroError> {
        ...
        tokio::fs::write(&self.vault_path, json.as_bytes())
            .await
            .map_err(|e| ElectroError::Vault(format!("failed to write vault file: {e}")))?;
    ...
    /// Parse a `vault://electro/<key>` URI and return the key portion.
    fn parse_vault_uri(uri: &str) -> Result<String, ElectroError> {
        let rest = uri
            .strip_prefix("vault://electro/")
            .ok_or_else(|| ElectroError::Vault(format!("invalid vault URI: {uri}")))?;

        if rest.is_empty() {
            return Err(ElectroError::Vault("vault URI has empty key".into()));
        }
...
trait PipeOk: Sized {
    fn pipe_ok(self) -> Result<Self, ElectroError> {
        Ok(self)
    }
}
impl<T> PipeOk for T {}
...
impl Vault for LocalVault {
    ...
    async fn get_secret(&self, key: &str) -> Result<Option<Vec<u8>>, ElectroError> {
        ...
        let nonce_bytes: [u8; 12] = engine
            .decode(&stored.nonce)
            .map_err(|e| ElectroError::Vault(format!("bad nonce base64: {e}")))?
            .try_into()
            .map_err(|_| ElectroError::Vault("nonce must be 12 bytes".into()))?;

        let ciphertext = engine
            .decode(&stored.ciphertext)
            .map_err(|e| ElectroError::Vault(format!("bad ciphertext base64: {e}")))?;
...
mod tests {
    ...
    async fn round_trip() {
        ...
        // resolve_uri
        let resolved = vault
            .resolve_uri("vault://electro/test/key")
            .await
            .unwrap()
            .unwrap();
    ...
    async fn resolve_uri_nonexistent_returns_none() {
        ...
        let result = vault.resolve_uri("vault://electro/missing").await.unwrap();
        assert!(result.is_none());
    }
    ...
    async fn resolve_uri_invalid_prefix_errors() {
        ...
        let result = vault.resolve_uri("http://electro/key").await;
        assert!(result.is_err());
    }
