//! Vault URI resolver — parses `vault://electro/<key>` URIs and delegates
...
const VAULT_AUTHORITY: &str = "electro";

/// Parsed vault URI.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct VaultUri {
    /// The authority portion (e.g. "electro").
    pub authority: String,
    /// The key path (everything after `vault://authority/`).
    pub key: String,
}

/// Parse a `vault://electro/key_name` URI.
///
/// Returns an error if the URI does not start with `vault://` or the
/// authority is not `electro`.
pub fn parse_vault_uri(uri: &str) -> Result<VaultUri, ElectroError> {
    let rest = uri
        .strip_prefix(VAULT_SCHEME)
        .ok_or_else(|| ElectroError::Vault(format!("not a vault URI: {uri}")))?;

    let (authority, key) = rest
        .split_once('/')
        .ok_or_else(|| ElectroError::Vault(format!("vault URI missing key path: {uri}")))?;

    if authority != VAULT_AUTHORITY {
        return Err(ElectroError::Vault(format!(
...
pub async fn resolve(vault: &dyn Vault, uri: &str) -> Result<Option<Vec<u8>>, ElectroError> {
...
mod tests {
    ...
    #[test]
    fn parse_valid() {
        let uri = parse_vault_uri("vault://electro/my/secret/key").unwrap();
        assert_eq!(uri.authority, "electro");
        assert_eq!(uri.key, "my/secret/key");
    }

    #[test]
    fn parse_simple_key() {
        let uri = parse_vault_uri("vault://electro/api_key").unwrap();
        assert_eq!(uri.key, "api_key");
    }

    #[test]
    fn reject_wrong_scheme() {
        assert!(parse_vault_uri("http://electro/key").is_err());
    }
    ...
    #[test]
    fn reject_empty_key() {
        assert!(parse_vault_uri("vault://electro/").is_err());
    }
    ...
    fn is_vault_uri_check() {
        assert!(is_vault_uri("vault://electro/key"));
    ...
    fn parse_nested_key_path() {
        let uri = parse_vault_uri("vault://electro/providers/anthropic/api_key").unwrap();
    ...
    fn reject_vault_uri_no_slash_after_authority() {
        // "vault://electro" has no trailing slash or key
        assert!(parse_vault_uri("vault://electro").is_err());
    }

    #[test]
    fn parse_vault_uri_with_special_chars_in_key() {
        let uri = parse_vault_uri("vault://electro/key-with_special.chars").unwrap();
