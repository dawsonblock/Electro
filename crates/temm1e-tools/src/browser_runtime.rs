    ...
    if browser_uses_remote() {
        ...
        Browser::connect(url.clone()).await.map_err(|e| {
            Temm1eError::Tool(format!(
                "Failed to connect to isolated remote browser at {url}. Start the browser sandbox compose stack or set {BROWSER_ISOLATION_MODE_ENV}=local intentionally. Error: {e}"
            ))
        })
    } else {
