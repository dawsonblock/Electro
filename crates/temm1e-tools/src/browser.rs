...
impl BrowserTool {
    ...
    async fn ensure_browser(&self) -> Result<Page, Temm1eError> {
        ...
        let proxy = std::env::var("TEMM1E_BROWSER_PROXY_SERVER").unwrap_or_default();
        if !remote_browser && !proxy.trim().is_empty() {
            ...
            let bypass = std::env::var("TEMM1E_BROWSER_PROXY_BYPASS").unwrap_or_default();
        ...
        if !remote_browser {
            ...
            let inherit_browser_session = std::env::var("TEMM1E_INHERIT_BROWSER_SESSION").unwrap_or_default() == "1";
                ...
                if !default_subdir.join("Cookies").exists() {
                    ...
                    if let Some(ref real) = real_profile {
                        ...
                        tracing::warn!(
                            ...
                            "Browser: inherited live Chrome profile because TEMM1E_INHERIT_BROWSER_SESSION=1"
                        );
                    } else {
                        tracing::info!("Browser: no Chrome profile found, using fresh profile");
                    }
                } else {
                    tracing::warn!(profile = %work_profile.display(), "Browser: reusing inherited work profile because TEMM1E_INHERIT_BROWSER_SESSION=1");
                }
            }
...
impl Tool for BrowserTool {
    ...
    fn description(&self) -> &str {
        "Control a stealth Chrome browser to navigate websites, click elements, type text, \
         ...
         - evaluate: Execute JavaScript and return the result (disabled by default; requires TEMM1E_BROWSER_ALLOW_EVAL=1)\n\
    ...
    async fn execute(
        ...
        match action {
            "navigate" => {
                ...
                let host = parsed
                    .host_str()
                    .ok_or_else(|| Temm1eError::Tool("Navigate URL must include a host".into()))?;
                ...
                if !current_url.is_empty() {
                    let final_url = validate_public_url(&current_url)
                        .map_err(|msg| Temm1eError::Tool(format!("Browser ended on blocked URL after navigation: {}", msg)))?;
                    ensure_resolved_host_is_public(&final_url)
                        .await
                        .map_err(|msg| Temm1eError::Tool(format!("Browser final URL resolution blocked: {}", msg)))?;
...
mod tests {
    ...
    fn sessions_dir_returns_correct_path() {
        ...
        assert!(
            path_str.ends_with(".temm1e/sessions"),
            "Sessions dir should end with .temm1e/sessions, got: {}",
            path_str
        );
    }
