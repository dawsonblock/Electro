...
//!   both `TEMM1E_SHELL_BACKEND=host` and `TEMM1E_ENABLE_HOST_SHELL=1`
...
fn host_shell_enabled() -> bool {
    matches!(
        std::env::var("TEMM1E_ENABLE_HOST_SHELL")
            .unwrap_or_default()
            .trim()
            .to_ascii_lowercase()
            .as_str(),
...
fn load_policy() -> ShellRunnerPolicy {
    ShellRunnerPolicy {
        backend: parse_backend(
            &std::env::var("TEMM1E_SHELL_BACKEND").unwrap_or_else(|_| "auto".to_string()),
        ),
        container_image: std::env::var("TEMM1E_SHELL_CONTAINER_IMAGE")
            .unwrap_or_else(|_| DEFAULT_CONTAINER_IMAGE.to_string())
            .trim()
            .to_string(),
        allow_network: env_truthy("TEMM1E_SHELL_ALLOW_NETWORK"),
        memory_mb: parse_u64_env("TEMM1E_SHELL_MEMORY_MB", DEFAULT_MEMORY_MB, 64, 8192),
        pids_limit: parse_u64_env("TEMM1E_SHELL_PIDS_LIMIT", DEFAULT_PIDS_LIMIT, 16, 4096),
        cpu_limit: parse_f64_env("TEMM1E_SHELL_CPU_LIMIT", DEFAULT_CPU_LIMIT, 0.25, 8.0),
        tmpfs_mb: parse_u64_env("TEMM1E_SHELL_TMPFS_MB", DEFAULT_TMPFS_MB, 16, 2048),
    }
}
    ...
    match policy.backend {
        ShellBackend::Docker => {
            if command_available("docker").await {
                ...
                Err("TEMM1E_SHELL_BACKEND=docker but the docker CLI is not available on PATH.".to_string())
            }
        }
        ShellBackend::Podman => {
            if command_available("podman").await {
                ...
                Err("TEMM1E_SHELL_BACKEND=podman but the podman CLI is not available on PATH.".to_string())
            }
        }
        ShellBackend::Host => {
            if host_shell_enabled() {
                ...
                Err("Direct host execution requires both TEMM1E_SHELL_BACKEND=host and TEMM1E_ENABLE_HOST_SHELL=1.".to_string())
            }
        }
        ShellBackend::Auto => {
            if command_available("docker").await {
                ...
                Err("No isolated shell runner is available. Install docker or podman, or explicitly opt into direct host execution with TEMM1E_SHELL_BACKEND=host and TEMM1E_ENABLE_HOST_SHELL=1.".to_string())
            }
        }
    }
}
...
fn validate_host_program(parsed: &ParsedCommand) -> Result<(), Temm1eError> {
...
fn passthrough_env_names() -> Vec<String> {
    ...
    for raw in std::env::var("TEMM1E_SHELL_PASSTHROUGH_ENV")
        .unwrap_or_default()
        .split(',')
    {
        let name = raw.trim();
...
fn prepare_host_runtime_dirs(
    ctx: &ToolContext,
) -> Result<(PathBuf, PathBuf, PathBuf, PathBuf, PathBuf), Temm1eError> {
    let home = ctx.workspace_path.join(".temm1e-host-home");
    let cache = ctx.workspace_path.join(".temm1e-host-cache");
    let config = ctx.workspace_path.join(".temm1e-host-config");
    let state = ctx.workspace_path.join(".temm1e-host-state");
    let tmp = ctx.workspace_path.join(".temm1e-host-tmp");
...
fn build_container_args(
    ...
    let mut args = vec![
        ...
        mount_arg,
        "--env".to_string(),
        "TEMM1E_WORKSPACE=/workspace".to_string(),
        "--env".to_string(),
        "HOME=/tmp".to_string(),
    ];
...
async fn run_host_command(
    ...
    command
        ...
        .env_clear()
        .env("TEMM1E_WORKSPACE", &ctx.workspace_path)
        .env("HOME", &home)
...
impl Tool for ShellTool {
    ...
    fn description(&self) -> &str {
        "Execute a simple command inside the session workspace. By default TEMM1E uses an isolated docker or podman runner with a workspace-only mount and network disabled. Direct host execution requires both TEMM1E_SHELL_BACKEND=host and TEMM1E_ENABLE_HOST_SHELL=1. When host fallback is used, TEMM1E clears the inherited environment, points HOME/XDG state into the workspace, and blocks launcher programs such as sh and bash. Metacharacters, pipes, redirection, and command chaining are blocked."
    }
