...
impl InteractiveBrowseSession {
    ...
    pub async fn new(browser: &Browser, service: &str, url: &str) -> Result<Self, Temm1eError> {
        let parsed = validate_public_url(url).map_err(Temm1eError::Tool)?;
        ...
        let current_url = page
            .url()
            .await
            .map_err(|e| Temm1eError::Tool(format!("Failed to get session URL: {}", e)))?
            .map(|u| u.to_string())
            .unwrap_or_default();
        if !current_url.is_empty() {
            let final_url = validate_public_url(&current_url)
                .map_err(|msg| Temm1eError::Tool(format!("Interactive browser session ended on blocked URL: {}", msg)))?;
            ensure_resolved_host_is_public(&final_url)
                .await
                .map_err(|msg| Temm1eError::Tool(format!("Interactive browser session final URL resolution blocked: {}", msg)))?;
    ...
    pub async fn launch(service: &str, url: &str) -> Result<Self, Temm1eError> {
        ...
        if !remote_browser {
            ...
            let force_clean_browser = std::env::var("TEMM1E_CLEAN_BROWSER").unwrap_or_default() == "1";
