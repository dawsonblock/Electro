...
//! `TEMM1E_ENABLE_CUSTOM_TOOLS=1` is explicitly set by the operator.
    ...
    async fn execute(
        ...
        if !custom_tools_enabled() {
            return Ok(ToolOutput {
                content: "Custom tool execution is disabled. Set TEMM1E_ENABLE_CUSTOM_TOOLS=1 only after manual review of persisted scripts.".to_string(),
                is_error: true,
            });
        }
    ...
    pub fn create_tool(
        ...
        if !custom_tools_enabled() {
            return Err(Temm1eError::Tool(
                "Custom tools are disabled. Set TEMM1E_ENABLE_CUSTOM_TOOLS=1 only after manual review and only in a trusted environment.".to_string(),
            ));
        }
...
impl Tool for SelfCreateTool {
    ...
    async fn execute(
        ...
        if !custom_tools_enabled() {
            return Ok(ToolOutput {
                content: "Custom tool creation and loading are disabled. Set TEMM1E_ENABLE_CUSTOM_TOOLS=1 only after manual review of scripts and tool metadata.".to_string(),
                is_error: true,
            });
        }
