//! ELECTRO Codex OAuth — OpenAI Codex subscription via OAuth PKCE
//!
//! Enables ELECTRO users to authenticate with their ChatGPT Plus/Pro subscription
...
//! to nothing — zero impact on the rest of ELECTRO.
//!
//! # Modules
//!
...
pub async fn login(headless: bool) -> Result<TokenStore, ElectroError> {
...
async fn login_headless(
    ...
    std::io::stdin()
        .read_line(&mut input)
        .map_err(|e| ElectroError::Auth(format!("Failed to read input: {}", e)))?;

    let input = input.trim();

    // Extract the code and state from the pasted URL
    let url =
        url::Url::parse(input).map_err(|e| ElectroError::Auth(format!("Invalid URL: {}", e)))?;

    let code = url
        .query_pairs()
        .find(|(k, _)| k == "code")
        .map(|(_, v)| v.to_string())
        .ok_or_else(|| ElectroError::Auth("No 'code' parameter in URL".to_string()))?;

    let received_state = url
        .query_pairs()
        .find(|(k, _)| k == "state")
        .map(|(_, v)| v.to_string())
        .ok_or_else(|| ElectroError::Auth("No 'state' parameter in URL".to_string()))?;
...
async fn exchange_code(
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| ElectroError::Auth(format!("Token exchange request failed: {}", e)))?;
...
fn find_available_port() -> Result<u16, ElectroError> {
    for port in 1455..1555 {
        if std::net::TcpListener::bind(format!("127.0.0.1:{}", port)).is_ok() {
            return Ok(port);
        }
    }
    Err(ElectroError::Auth(
