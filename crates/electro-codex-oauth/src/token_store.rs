...
//!
//! Tokens are stored in `~/.electro/oauth.json`. Access tokens expire in ~1 hour
...
impl TokenStore {
    ...
    /// Load tokens from ~/.electro/oauth.json
    pub fn load() -> Result<Self, ElectroError> {
        let path = Self::default_path();
        let content = std::fs::read_to_string(&path).map_err(|e| {
            ElectroError::Auth(format!(
                "No OAuth tokens found at {}. Run `electro auth login` first. ({})",
                path.display(),
                e
            ))
        })?;
        let tokens: CodexOAuthTokens = serde_json::from_str(&content)
            .map_err(|e| ElectroError::Auth(format!("Failed to parse OAuth tokens: {}", e)))?;
        Ok(Self {
            path,
            tokens: Mutex::new(tokens),
            client: reqwest::Client::new(),
    ...
    pub async fn get_access_token(&self) -> Result<String, ElectroError> {
    ...
    /// Save tokens to disk.
    pub fn save_to_disk(&self, tokens: &CodexOAuthTokens) -> Result<(), ElectroError> {
        let dir = self.path.parent().unwrap_or(std::path::Path::new("."));
        std::fs::create_dir_all(dir)
            .map_err(|e| ElectroError::Auth(format!("Failed to create dir: {}", e)))?;
        let content = serde_json::to_string_pretty(tokens)
            .map_err(|e| ElectroError::Auth(format!("Failed to serialize tokens: {}", e)))?;
        std::fs::write(&self.path, content)
            .map_err(|e| ElectroError::Auth(format!("Failed to write tokens: {}", e)))?;
    ...
    async fn refresh_token(
        client: &reqwest::Client,
        refresh_token: &str,
    ) -> Result<RefreshResponse, ElectroError> {
        let params = [
        ...
        let resp = client
            ...
            .send()
            .await
            .map_err(|e| ElectroError::Auth(format!("Token refresh request failed: {}", e)))?;
        ...
        let token_resp: TokenResponse = resp
            .json()
            .await
            .map_err(|e| ElectroError::Auth(format!("Failed to parse refresh response: {}", e)))?;
    ...
    pub fn delete() -> Result<(), ElectroError> {
        let path = Self::default_path();
        if path.exists() {
            std::fs::remove_file(&path)
                .map_err(|e| ElectroError::Auth(format!("Failed to delete tokens: {}", e)))?;
        }
        Ok(())
    }
...
mod tests {
    ...
    fn default_path_ends_with_oauth_json() {
        ...
        assert!(path.to_string_lossy().contains(".electro"));
    }
