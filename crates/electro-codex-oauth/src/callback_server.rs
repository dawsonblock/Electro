...
pub async fn wait_for_callback(
    ...
    timeout_secs: u64,
    port: Option<u16>,
) -> Result<(CallbackResult, u16), ElectroError> {
    let (tx, rx) = oneshot::channel::<Result<CallbackResult, ElectroError>>();
            ...
            async move {
                ...
                if let Some(sender) = guard.take() {
                    if let Some(error) = params.error {
                        ...
                        let _ = sender.send(Err(ElectroError::Auth(format!(
                            "OAuth error: {} — {}",
                            error, desc
                        ))));
                    ...
                    match (params.code, params.state) {
                        (Some(code), Some(state)) => {
                            if state != expected {
                                let _ = sender.send(Err(ElectroError::Auth(
    ...
    let listener = tokio::net::TcpListener::bind(&addr)
        .await
        .map_err(|e| ElectroError::Auth(format!("Failed to bind callback server: {}", e)))?;
    ...
    let result = tokio::time::timeout(std::time::Duration::from_secs(timeout_secs), rx)
        ...
        .map_err(|_| ElectroError::Auth("OAuth callback channel closed".to_string()))?;
...
fn success_page() -> String {
    r#"<!DOCTYPE html>
<html><head><title>ELECTRO — Authenticated</title></head>
...
<p>You can close this tab and return to ELECTRO.</p>
</div></body></html>"#
        .to_string()
}
...
fn error_page(error: &str, description: &str) -> String {
    format!(
        r#"<!DOCTYPE html>
<html><head><title>ELECTRO — Auth Error</title></head>
<body style="font-family:system-ui;display:flex;justify-content:center;align-items:center;height:100vh;margin:0;background:#0a0a0a;color:#e0e0e0;">
<div style="text-align:center;max-width:400px;">
