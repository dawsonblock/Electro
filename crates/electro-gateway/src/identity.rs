...
use electro_core::{AuthResult, Identity};
...
impl OAuthIdentityManager {
    ...
    pub async fn start_oauth_flow(
        ...
        let config = self
            ...
            .ok_or_else(|| ElectroError::Auth(format!("Unknown OAuth provider: {}", provider)))?
            .clone();
    ...
    pub async fn complete_oauth_flow(
        ...
        state: &str,
        code: &str,
    ) -> Result<OAuthToken, ElectroError> {
        // Look up and remove the pending flow
        let flow = {
            let mut flows = self.pending_flows.write().await;
            flows.remove(state).ok_or_else(|| {
                ElectroError::Auth("Invalid or expired OAuth state parameter".to_string())
            })?
        };
        ...
        let config = self
            ...
            .ok_or_else(|| {
                ElectroError::Auth(format!("Provider config not found: {}", flow.provider))
            })?
            .clone();
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| ElectroError::Auth(format!("Token exchange HTTP error: {}", e)))?;
        ...
        let token_resp: TokenResponse = resp
            .json()
            .await
            .map_err(|e| ElectroError::Auth(format!("Failed to parse token response: {}", e)))?;
    ...
    pub async fn refresh_token(
        ...
        let (refresh_tok, config) = {
            ...
            let user = users
                .get(user_id)
                .ok_or_else(|| ElectroError::Auth(format!("User not found: {}", user_id)))?;
            ...
            let refresh = current
                .refresh_token
                .clone()
                .ok_or_else(|| ElectroError::Auth("No refresh token available".to_string()))?;
            let cfg = self
                .find_provider_config(provider)
                .ok_or_else(|| ElectroError::Auth(format!("Unknown OAuth provider: {}", provider)))?
                .clone();
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| ElectroError::Auth(format!("Token refresh HTTP error: {}", e)))?;
...
impl Identity for OAuthIdentityManager {
    ...
    async fn has_permission(&self, user_id: &str, permission: &str) -> Result<bool, ElectroError> {
        ...
        match users.get(user_id) {
            ...
            None => Err(ElectroError::Auth(format!("User not found: {}", user_id))),
        }
    }
