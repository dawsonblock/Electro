...
impl FileTransfer for CliChannel {
    async fn receive_file(&self, msg: &InboundMessage) -> Result<Vec<ReceivedFile>, ElectroError> {
        ...
        for att in &msg.attachments {
            ...
            let data = tokio::fs::read(path).await.map_err(|e| {
                ElectroError::FileTransfer(format!("Failed to read {}: {e}", path.display()))
            })?;
            let size = data.len();
    ...
    async fn send_file(&self, _chat_id: &str, file: OutboundFile) -> Result<(), ElectroError> {
        ...
        tokio::fs::create_dir_all(&self.workspace)
            .await
            .map_err(|e| ElectroError::FileTransfer(format!("Failed to create workspace: {e}")))?;
        tokio::fs::write(&dest, &data)
            .await
            .map_err(|e| ElectroError::FileTransfer(format!("Failed to write file: {e}")))?;
