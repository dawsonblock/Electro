...
/// Return the path to `~/.electro/slack_allowlist.toml`.
...
fn save_allowlist_file(data: &AllowlistFile) -> Result<(), ElectroError> {
    ...
    std::fs::write(&path, content)
        .map_err(|e| ElectroError::Channel(format!("Failed to write Slack allowlist file: {e}")))?;
...
pub struct SlackChannel {
    ...
    /// Empty at startup = deny all unless explicit bootstrap is enabled.
    allowlist: Arc<RwLock<Vec<String>>>,
...
impl SlackChannel {
    ...
    ///
    /// If a persisted allowlist exists at `~/.electro/slack_allowlist.toml`,
    /// it is loaded and merged with any entries from the config file.
    pub fn new(config: &ChannelConfig) -> Result<Self, ElectroError> {
        let token = config
            .token
            .clone()
            .ok_or_else(|| ElectroError::Config("Slack channel requires a bot token".into()))?;
...
impl Channel for SlackChannel {
    ...
    async fn start(&mut self) -> Result<(), ElectroError> {
        ...
        let auth_response = self
            ...
            .send()
            .await
            .map_err(|e| ElectroError::Channel(format!("Failed to call Slack auth.test: {e}")))?;

        let auth_result: SlackApiResponse<AuthTestData> =
            auth_response.json().await.map_err(|e| {
                ElectroError::Channel(format!("Failed to parse Slack auth.test response: {e}"))
            })?;

        if !auth_result.ok {
            return Err(ElectroError::Channel(format!(
    ...
    async fn send_message(&self, msg: OutboundMessage) -> Result<(), ElectroError> {
        ...
        for chunk in chunks {
            ...
            let response = self
                ...
                .send()
                .await
                .map_err(|e| ElectroError::Channel(format!("Failed to send Slack message: {e}")))?;
    ...
    async fn delete_message(&self, chat_id: &str, message_id: &str) -> Result<(), ElectroError> {
        let response: serde_json::Value = self
            ...
            .send()
            .await
            .map_err(|e| ElectroError::Channel(format!("Failed to call Slack chat.delete: {}", e)))?
            .json()
            .await
            .map_err(|e| {
                ElectroError::Channel(format!("Failed to parse Slack chat.delete response: {}", e))
            })?;
...
impl FileTransfer for SlackChannel {
    async fn receive_file(&self, msg: &InboundMessage) -> Result<Vec<ReceivedFile>, ElectroError> {
        ...
        for att in &msg.attachments {
            ...
            let response = self
                ...
                .await
                .map_err(|e| {
                    ElectroError::FileTransfer(format!("Failed to download Slack attachment: {e}"))
                })?;

            let data = response.bytes().await.map_err(|e| {
                ElectroError::FileTransfer(format!("Failed to read Slack attachment bytes: {e}"))
            })?;
    ...
    async fn send_file(&self, chat_id: &str, file: OutboundFile) -> Result<(), ElectroError> {
        let data = match &file.data {
            ...
            FileData::Url(url) => {
                let response = reqwest::get(url).await.map_err(|e| {
                    ElectroError::FileTransfer(format!("Failed to download file from URL: {e}"))
                })?;
                response
                    .bytes()
                    .await
                    .map_err(|e| {
                        ElectroError::FileTransfer(format!("Failed to read file bytes: {e}"))
                    })?
                    .to_vec()
            }
        };
        ...
        let part = reqwest::multipart::Part::bytes(data)
            ...
            .map_err(|e| ElectroError::FileTransfer(format!("Invalid MIME type: {e}")))?;
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                ElectroError::FileTransfer(format!("Failed to upload file to Slack: {e}"))
            })?;

        let result: SlackApiResponse<serde_json::Value> = response.json().await.map_err(|e| {
            ElectroError::FileTransfer(format!("Failed to parse Slack files.upload response: {e}"))
        })?;
                ...
                // Optional first-user bootstrap (disabled by default).
                {
                    ...
                    if list.is_empty() && allow_first_user_bootstrap_enabled() {
                        ...
                        *adm = Some(user_id.clone());
                        tracing::warn!(
                            user_id = %user_id,
                            "Bootstrapped first Slack user as admin because ELECTRO_ALLOW_FIRST_USER_BOOTSTRAP=1"
                        );
                        drop(list);
                        drop(adm);
                        if let Err(e) = persist_allowlist(&allowlist, &admin) {
                            tracing::error!(
                                error = %e,
                                "Failed to persist Slack allowlist after bootstrap"
                            );
                        }
                    }
                }
...
async fn fetch_bot_conversations(
    client: &reqwest::Client,
    token: &str,
) -> Result<Vec<SlackConversation>, ElectroError> {
    let response = client
        ...
        .await
        .map_err(|e| {
            ElectroError::Channel(format!("Failed to call Slack conversations.list: {e}"))
        })?;
...
async fn fetch_conversation_history(
    ...
    channel_id: &str,
    oldest: &str,
) -> Result<Vec<SlackMessage>, ElectroError> {
    let response = client
        ...
        .await
        .map_err(|e| {
            ElectroError::Channel(format!("Failed to call Slack conversations.history: {e}"))
        })?;
