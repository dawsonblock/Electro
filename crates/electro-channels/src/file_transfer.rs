...
use electro_core::types::file::{FileData, OutboundFile, ReceivedFile};
...
pub async fn save_received_file(
    file: &ReceivedFile,
    workspace: &Path,
) -> Result<PathBuf, ElectroError> {
    // Ensure workspace directory exists
    tokio::fs::create_dir_all(workspace)
        .await
        .map_err(|e| ElectroError::FileTransfer(format!("Failed to create workspace dir: {e}")))?;
    ...
    tokio::fs::write(&dest, &file.data)
        .await
        .map_err(|e| ElectroError::FileTransfer(format!("Failed to write file {safe_name}: {e}")))?;
...
pub async fn read_file_for_sending(path: &Path) -> Result<OutboundFile, ElectroError> {
    let data = tokio::fs::read(path).await.map_err(|e| {
        ElectroError::FileTransfer(format!("Failed to read file {}: {e}", path.display()))
    })?;
