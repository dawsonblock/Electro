...
/// `~/.electro/discord_allowlist.toml`.
...
/// Return the path to `~/.electro/discord_allowlist.toml`.
...
impl DiscordChannel {
    ...
    ///
    /// If a persisted allowlist exists at `~/.electro/discord_allowlist.toml`,
    /// it is loaded and merged with any entries from the config file.
    pub fn new(config: &ChannelConfig) -> Result<Self, ElectroError> {
        let token = config
            .token
            .clone()
            .ok_or_else(|| ElectroError::Config("Discord channel requires a bot token".into()))?;
...
impl Channel for DiscordChannel {
    ...
    async fn send_message(&self, msg: OutboundMessage) -> Result<(), ElectroError> {
        let http = {
            ...
            guard
                .clone()
                .ok_or_else(|| ElectroError::Channel("Discord client not connected yet".into()))?
        };
        ...
        let channel_id: ChannelId =
            msg.chat_id
                ...
                .map(ChannelId::new)
                .map_err(|_| {
                    ElectroError::Channel(format!("Invalid Discord channel_id: {}", msg.chat_id))
                })?;
        ...
        for chunk in chunks {
            ...
            channel_id.send_message(&http, builder).await.map_err(|e| {
                ElectroError::Channel(format!("Failed to send Discord message: {e}"))
            })?;
        }
    ...
    async fn delete_message(&self, chat_id: &str, message_id: &str) -> Result<(), ElectroError> {
        ...
        let channel_id: ChannelId = chat_id.parse::<u64>().map(ChannelId::new).map_err(|_| {
            ElectroError::Channel(format!("Invalid Discord channel_id: {}", chat_id))
        })?;
        ...
        let msg_id: serenity::all::MessageId = message_id
            ...
            .map_err(|_| {
                ElectroError::Channel(format!("Invalid Discord message_id: {}", message_id))
            })?;
...
impl FileTransfer for DiscordChannel {
    async fn receive_file(&self, msg: &InboundMessage) -> Result<Vec<ReceivedFile>, ElectroError> {
        ...
        for att in &msg.attachments {
            ...
            let client = reqwest::Client::builder()
                ...
                .build()
                .map_err(|e| {
                    ElectroError::FileTransfer(format!("Failed to build HTTP client: {e}"))
                })?;
            let response = client.get(url).send().await.map_err(|e| {
                ElectroError::FileTransfer(format!("Failed to download Discord attachment: {e}"))
            })?;

            let data = response.bytes().await.map_err(|e| {
                ElectroError::FileTransfer(format!("Failed to read Discord attachment bytes: {e}"))
            })?;
    ...
    async fn send_file(&self, chat_id: &str, file: OutboundFile) -> Result<(), ElectroError> {
        ...
        let channel_id: ChannelId = chat_id
            .parse::<u64>()
            .map(ChannelId::new)
            .map_err(|_| ElectroError::Channel(format!("Invalid Discord channel_id: {chat_id}")))?;
        ...
        channel_id
            ...
            .await
            .map_err(|e| ElectroError::FileTransfer(format!("Failed to send Discord file: {e}")))?;
    ...
    async fn message(&self, ctx: Context, msg: Message) {
        ...
        // Optional first-user bootstrap (disabled by default)
        {
            let mut list = match self.allowlist.write() {
                ...
                Err(poisoned) => {
                    tracing::error!(
                        "Discord allowlist RwLock poisoned in bootstrap check, recovering"
                    );
                    poisoned.into_inner()
                }
            };
            if list.is_empty() && allow_first_user_bootstrap_enabled() {
                ...
                let mut adm = match self.admin.write() {
                    ...
                    Err(poisoned) => {
                        tracing::error!(
                            "Discord admin RwLock poisoned in bootstrap check, recovering"
                        );
                        poisoned.into_inner()
                    }
                };
                *adm = Some(user_id.clone());
                tracing::warn!(
                    user_id = %user_id,
                    username = ?username,
                    "Bootstrapped first Discord user as admin because ELECTRO_ALLOW_FIRST_USER_BOOTSTRAP=1"
                );
                drop(list);
                drop(adm);
                if let Err(e) = persist_allowlist(&self.allowlist, &self.admin) {
                    tracing::error!(
                        error = %e,
                        "Failed to persist Discord allowlist after bootstrap"
                    );
                }
            }
        }
