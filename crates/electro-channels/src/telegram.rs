...
/// On-disk representation of the allowlist stored at `~/.electro/allowlist.toml`.
...
/// Return the path to `~/.electro/allowlist.toml`.
fn allow_first_user_bootstrap_enabled() -> bool {
    matches!(
        std::env::var("ELECTRO_ALLOW_FIRST_USER_BOOTSTRAP")
            .unwrap_or_default()
            .trim()
            .to_ascii_lowercase()
            .as_str(),
...
/// Save the allowlist to disk. Creates `~/.electro/` if needed.
fn save_allowlist_file(data: &AllowlistFile) -> Result<(), ElectroError> {
    ...
    let path = paths::allowlist_file().map_err(|e| {
        ElectroError::Channel(format!("Cannot determine home directory for allowlist: {e}"))
    })?;
    let content = toml::to_string_pretty(data)
        .map_err(|e| ElectroError::Channel(format!("Failed to serialize allowlist: {e}")))?;
    std::fs::write(&path, content)
        .map_err(|e| ElectroError::Channel(format!("Failed to write allowlist file: {e}")))?;
...
pub struct TelegramChannel {
    ...
    /// user is manually allowlisted, legacy bootstrap is enabled, or the value is loaded from the persisted allowlist file.
...
impl TelegramChannel {
    ...
    ///
    /// If a persisted allowlist exists at `~/.electro/allowlist.toml`, it is
    ...
    pub fn new(config: &ChannelConfig) -> Result<Self, ElectroError> {
        let token = config
            .token
            .clone()
            .ok_or_else(|| ElectroError::Config("Telegram channel requires a bot token".into()))?;
    ...
    /// An empty allowlist means no one is whitelisted yet (legacy bootstrap
...
impl Channel for TelegramChannel {
    ...
    async fn start(&mut self) -> Result<(), ElectroError> {
        ...
        bot.get_me().await.map_err(|e| {
            ElectroError::Channel(format!("Invalid Telegram bot token — getMe failed: {e}"))
        })?;
    ...
    async fn send_message(&self, msg: OutboundMessage) -> Result<(), ElectroError> {
        let bot = self
            .bot
            .as_ref()
            .ok_or_else(|| ElectroError::Channel("Telegram bot not started".into()))?;

        let chat_id: ChatId = msg
            .chat_id
            .parse::<i64>()
            .map(ChatId)
            .map_err(|_| ElectroError::Channel(format!("Invalid chat_id: {}", msg.chat_id)))?;
        ...
        request
            .await
            .map_err(|e| ElectroError::Channel(format!("Failed to send Telegram message: {e}")))?;
    ...
    async fn delete_message(&self, chat_id: &str, message_id: &str) -> Result<(), ElectroError> {
        let bot = self
            .bot
            .as_ref()
            .ok_or_else(|| ElectroError::Channel("Telegram bot not started".into()))?;
        ...
        let tg_chat_id: ChatId = chat_id
            .parse::<i64>()
            .map(ChatId)
            .map_err(|_| ElectroError::Channel(format!("Invalid chat_id: {}", chat_id)))?;

        let msg_id: teloxide::types::MessageId =
            teloxide::types::MessageId(message_id.parse::<i32>().map_err(|_| {
                ElectroError::Channel(format!("Invalid message_id: {}", message_id))
            })?);

        bot.delete_message(tg_chat_id, msg_id).await.map_err(|e| {
            ElectroError::Channel(format!(
...
impl FileTransfer for TelegramChannel {
    async fn receive_file(&self, msg: &InboundMessage) -> Result<Vec<ReceivedFile>, ElectroError> {
        ...
        for att in &msg.attachments {
            ...
            let tg_file = bot
                .get_file(file_id)
                .await
                .map_err(|e| ElectroError::FileTransfer(format!("Failed to get file info: {e}")))?;
            ...
            bot.download_file(&tg_file.path, &mut buf)
                .await
                .map_err(|e| ElectroError::FileTransfer(format!("Failed to download file: {e}")))?;
    ...
    async fn send_file(&self, chat_id: &str, file: OutboundFile) -> Result<(), ElectroError> {
        ...
        let input_file = match &file.data {
            ...
            FileData::Url(url) => {
                let parsed = url
                    .parse::<url::Url>()
                    .map_err(|e| ElectroError::FileTransfer(format!("Invalid file URL: {e}")))?;
        ...
        request
            .await
            .map_err(|e| ElectroError::FileTransfer(format!("Failed to send document: {e}")))?;
    ...
    async fn send_file_stream(
        ...
        Err(ElectroError::FileTransfer(format!(
...
async fn bot_reply(bot: &Bot, chat_id: ChatId, text: &str) -> Result<(), ElectroError> {
    bot.send_message(chat_id, text)
        .await
        .map_err(|e| ElectroError::Channel(format!("Failed to send admin-command reply: {e}")))?;
    Ok(())
}
...
async fn handle_telegram_message(
    ...
    // ── Optional first-user bootstrap (disabled by default) ───────
    {
        let mut list = match allowlist.write() {
            Ok(guard) => guard,
            Err(poisoned) => {
                tracing::error!("Allowlist RwLock poisoned in bootstrap check, recovering");
                poisoned.into_inner()
            }
        };
        if list.is_empty() && allow_first_user_bootstrap_enabled() {
            list.push(user_id.clone());
            let mut adm = match admin.write() {
                Ok(guard) => guard,
                Err(poisoned) => {
                    tracing::error!("Admin RwLock poisoned in bootstrap check, recovering");
                    poisoned.into_inner()
                }
            };
            *adm = Some(user_id.clone());
            tracing::warn!(user_id = %user_id, username = ?username, "Bootstrapped first Telegram user as admin because ELECTRO_ALLOW_FIRST_USER_BOOTSTRAP=1");
            drop(list);
            drop(adm);
            if let Err(e) = persist_allowlist(&allowlist, &admin) {
                tracing::error!(error = %e, "Failed to persist allowlist after bootstrap");
            }
        }
    }
    ...
    if !is_allowed {
        ...
        if allow_first_user_bootstrap_enabled() {
            return Ok(());
        }
        bot_reply(bot, chat_id, "Access denied. Add your Telegram user ID to ~/.electro/allowlist.toml or set ELECTRO_ALLOW_FIRST_USER_BOOTSTRAP=1 for one-time bootstrap.").await?;
        return Ok(());
    }
    ...
    tx.send(inbound)
        .await
        .map_err(|_| ElectroError::Channel("Inbound message receiver dropped".into()))?;
...
mod tests {
    ...
    fn telegram_allowlist_matches_user_ids() {
        ...
        // allowlist file at ~/.electro/allowlist.toml
        {
            let mut list = channel.allowlist.write().unwrap();
