...
pub trait DockerClient: Send + Sync {
    ...
    async fn stop_container(&self, id: &str) -> Result<(), ElectroError>;

    /// DELETE /containers/{id}?force=true
    async fn remove_container(&self, id: &str) -> Result<(), ElectroError>;

    /// GET /containers/{id}/json — returns the container status string.
    async fn inspect_container(&self, id: &str) -> Result<ContainerInspect, ElectroError>;
}
...
impl Orchestrator for DockerOrchestrator {
    ...
    async fn scale(&self, instance: &AgentInstance, replicas: u32) -> Result<(), ElectroError> {
        if replicas == 0 {
            return Err(ElectroError::Internal(
...
impl Orchestrator for KubernetesOrchestrator {
    async fn provision(&self, _spec: AgentSpec) -> Result<AgentInstance, ElectroError> {
        Err(ElectroError::Internal(
            "Kubernetes orchestrator not yet implemented".to_string(),
        ))
    }

    async fn scale(&self, _instance: &AgentInstance, _replicas: u32) -> Result<(), ElectroError> {
        Err(ElectroError::Internal(
            "Kubernetes orchestrator not yet implemented".to_string(),
        ))
    }

    async fn destroy(&self, _instance: &AgentInstance) -> Result<(), ElectroError> {
        Err(ElectroError::Internal(
            "Kubernetes orchestrator not yet implemented".to_string(),
        ))
    }

    async fn health(&self, _instance: &AgentInstance) -> Result<bool, ElectroError> {
        Err(ElectroError::Internal(
            "Kubernetes orchestrator not yet implemented".to_string(),
        ))
    }
...
) -> Result<Box<dyn Orchestrator>, ElectroError> {
    match backend {
        "docker" => {
            ...
            let _max_instances: u32 = config
                .get("max_instances")
                .and_then(|v| v.parse().ok())
                .unwrap_or(10);

            Err(ElectroError::Config(format!(
                "Factory backend 'docker' is disabled because this crate does not ship a real Docker client. Construct DockerOrchestrator::new(...) with an injected DockerClient for host '{}' instead.",
                docker_host
            )))
        }
        "kubernetes" => {
            ...
            Err(ElectroError::Config(format!(
                "Factory backend 'kubernetes' is disabled because KubernetesOrchestrator is a stub. Refusing to construct placeholder orchestrator for kubeconfig '{}'.",
                kubeconfig
            )))
        }
        other => Err(ElectroError::Config(format!(
...
impl DockerClient for UnimplementedDockerClient {
    ...
    async fn start_container(&self, _id: &str) -> Result<(), ElectroError> {
        Err(ElectroError::Internal(
            "No DockerClient configured".to_string(),
        ))
    }

    async fn stop_container(&self, _id: &str) -> Result<(), ElectroError> {
        Err(ElectroError::Internal(
            "No DockerClient configured".to_string(),
        ))
    }

    async fn remove_container(&self, _id: &str) -> Result<(), ElectroError> {
        Err(ElectroError::Internal(
            "No DockerClient configured".to_string(),
        ))
    }

    async fn inspect_container(&self, _id: &str) -> Result<ContainerInspect, ElectroError> {
        Err(ElectroError::Internal(
            "No DockerClient configured".to_string(),
        ))
    }
...
mod tests {
    ...
    impl DockerClient for MockDockerClient {
        async fn create_container(
            ...
            if self.fail_create {
                return Err(ElectroError::Internal("Docker create failed".to_string()));
            }
        ...
        async fn stop_container(&self, _id: &str) -> Result<(), ElectroError> {
            if self.fail_stop {
                return Err(ElectroError::Internal("Docker stop failed".to_string()));
            }
            Ok(())
        }
