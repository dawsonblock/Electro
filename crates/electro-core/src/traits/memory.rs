...
pub trait Memory: Send + Sync {
    ...
    async fn lambda_store(&self, _entry: LambdaMemoryEntry) -> Result<(), ElectroError> {
        Ok(())
    }
    ...
    async fn lambda_touch(&self, _hash: &str) -> Result<(), ElectroError> {
        Ok(())
    }
    ...
    async fn lambda_gc(&self, _now_epoch: u64, _max_age_secs: u64) -> Result<usize, ElectroError> {
        Ok(0)
    }
}
