...
use crate::types::config::ElectroConfig;
...
impl TenantManager {
    ...
    pub fn ensure_workspace(&self, tenant_id: &TenantId) -> Result<(), ElectroError> {
        let workspace = self.workspace_path(tenant_id);
        let vault = self.vault_path(tenant_id);

        std::fs::create_dir_all(&workspace).map_err(|e| {
            ElectroError::Io(std::io::Error::new(
                e.kind(),
                format!(
...
impl Tenant for TenantManager {
    ...
    async fn check_rate_limit(&self, tenant_id: &TenantId) -> Result<bool, ElectroError> {
        ...
        let mut limits = self
            .rate_limits
            .write()
            .map_err(|e| ElectroError::Internal(format!("Rate-limit lock poisoned: {e}")))?;
...
/// directory.
pub fn create_tenant_manager(config: &ElectroConfig) -> TenantManager {
    ...
    // added to ElectroConfig in a follow-up.
    ...
    if config.electro.tenant_isolation {
...
mod tests {
    ...
    fn factory_with_tenant_isolation_enabled() {
        let config = ElectroConfig {
            electro: crate::types::config::ElectroSection {
