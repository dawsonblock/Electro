...
fn config_paths() -> Vec<PathBuf> {
    ...
    paths.push(PathBuf::from("electro.toml"));

    paths
}

/// Load configuration from discovered config files, merging in order
pub fn load_config(explicit_path: Option<&Path>) -> Result<ElectroConfig, ElectroError> {
    ...
    if config_content.is_empty() {
        return Ok(ElectroConfig::default());
    }
    ...
    if let Ok(config) = toml::from_str::<ElectroConfig>(&expanded) {
        return Ok(config);
    }

    // Try YAML (OpenClaw compat)
    if let Ok(config) = serde_yaml::from_str::<ElectroConfig>(&expanded) {
        return Ok(config);
    }

    Err(ElectroError::Config(
...
pub fn load_agent_config(
...
) -> Result<Option<AgentAccessibleConfig>, ElectroError> {
    let mut config_content = String::new();
    ...
    let agent_config: AgentAccessibleConfig = toml::from_str(&expanded)
        .map_err(|e| ElectroError::Config(format!("Failed to parse agent config: {e}")))?;
...
) -> Result<ElectroConfig, ElectroError> {
...
"#;
    ...
    fn test_env_var_expansion_in_config() {
        std::env::set_var("ELECTRO_TEST_API_KEY", "expanded-key-value");
        let toml_content = r#"
[provider]
name = "anthropic"
api_key = "${ELECTRO_TEST_API_KEY}"
"#;
        let tmp = tempfile::NamedTempFile::new().unwrap();
        std::fs::write(tmp.path(), toml_content).unwrap();
        ...
        std::env::remove_var("ELECTRO_TEST_API_KEY");
    }
    ...
    fn test_missing_config_file() {
        let result = load_config(Some(std::path::Path::new(
            "/tmp/nonexistent_electro_config_12345.toml",
        )));
        assert!(result.is_err());
    }
...
[provider]
name = "anthropic"
api_key = "${NONEXISTENT_ELECTRO_VAR_99999}"
"#;
        let tmp = tempfile::NamedTempFile::new().unwrap();
        std::fs::write(tmp.path(), toml_content).unwrap();
...
"#;
    ...
    fn test_default_agent_config_path() {
        ...
        assert!(path.is_some());
        let p = path.unwrap();
        assert!(p.to_string_lossy().contains(".electro"));
