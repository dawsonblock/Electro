...
mod tests {
    ...
    fn test_expand_env_vars() {
        std::env::set_var("TEST_ELECTRO_VAR", "hello");
        assert_eq!(expand_env_vars("${TEST_ELECTRO_VAR}"), "hello");
        assert_eq!(
            expand_env_vars("prefix_${TEST_ELECTRO_VAR}_suffix"),
            "prefix_hello_suffix"
        );
        assert_eq!(expand_env_vars("no_vars_here"), "no_vars_here");
        assert_eq!(expand_env_vars("${NONEXISTENT_VAR}"), "");
        std::env::remove_var("TEST_ELECTRO_VAR");
    }
    ...
    fn test_multiple_vars_in_one_string() {
        std::env::set_var("ELECTRO_A", "alpha");
        std::env::set_var("ELECTRO_B", "beta");
        let result = expand_env_vars("${ELECTRO_A}:${ELECTRO_B}");
        assert_eq!(result, "alpha:beta");
        std::env::remove_var("ELECTRO_A");
        std::env::remove_var("ELECTRO_B");
    }
    ...
    fn test_nested_braces_not_recursive() {
        ...
        std::env::set_var("ELECTRO_INNER", "value");
        let result = expand_env_vars("${${ELECTRO_INNER}}");
        // The regex matches ${${ELECTRO_INNER} with var name "${ELECTRO_INNER"
        // which doesn't exist, so it expands to "" followed by the trailing "}"
        assert_eq!(result, "}");
        std::env::remove_var("ELECTRO_INNER");
    }
}
