...
use super::error::ElectroError;

/// Electro's operational personality mode
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
#[serde(rename_all = "lowercase")]
pub enum ElectroMode {
...
impl std::fmt::Display for ElectroMode {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ElectroMode::Play => write!(f, "PLAY :3"),
            ElectroMode::Work => write!(f, "WORK >:3"),
            ElectroMode::Pro => write!(f, "PRO"),
            ElectroMode::None => write!(f, "NONE"),
        }
    }
}

/// Top-level ELECTRO configuration
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct ElectroConfig {
    /// Electro's personality mode (play or work). Defaults to play.
    #[serde(default)]
    pub mode: ElectroMode,
    #[serde(default)]
...
impl Default for ElectroSection {
    fn default() -> Self {
        Self {
            mode: "auto".to_string(),
...
/// Agent-accessible configuration — the safe subset of ElectroConfig that
...
impl AgentAccessibleConfig {
    /// Extract agent-accessible config from a full ElectroConfig.
    pub fn from_master(config: &ElectroConfig) -> Self {
        Self {
    ...
    pub fn apply_to(&self, config: &mut ElectroConfig) {
    ...
    pub fn validate(&self) -> Result<(), ElectroError> {
        ...
        if !valid_levels.contains(&self.observability.log_level.to_lowercase().as_str()) {
            return Err(ElectroError::Config(format!(
    ...
    pub fn save(&self, path: &Path) -> Result<(), ElectroError> {
        self.validate()?;

        let toml_str = toml::to_string_pretty(self)
            .map_err(|e| ElectroError::Config(format!("Failed to serialize agent config: {e}")))?;

        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                ElectroError::Config(format!("Failed to create config directory: {e}"))
            })?;
        }

        std::fs::write(path, toml_str)
            .map_err(|e| ElectroError::Config(format!("Failed to write agent config: {e}")))?;
...
mod tests {
    ...
    fn config_serde_roundtrip() {
        let config = ElectroConfig {
            mode: ElectroMode::Play,
            electro: ElectroSection {
        ...
        let restored: ElectroConfig = toml::from_str(&toml_str).unwrap();
        assert_eq!(restored.electro.mode, "cloud");
        assert!(restored.electro.tenant_isolation);
    ...
    fn agent_config_apply_to_master() {
        let mut master = ElectroConfig::default();
    ...
    fn browser_timeout_secs_in_full_config() {
        ...
        "#;
        let config: ElectroConfig = toml::from_str(toml_str).unwrap();
    ...
    fn browser_timeout_secs_default_in_full_config() {
        ...
        let config = ElectroConfig::default();
