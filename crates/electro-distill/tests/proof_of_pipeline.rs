...
//!
//! Run with: cargo test -p electro-distill --test proof_of_pipeline -- --nocapture

use std::sync::Arc;
use electro_distill::collector::{EigenTuneCollector, EigenTunePairData};
use electro_distill::config::EigenTuneConfig;
use electro_distill::stats::{cusum::Cusum, entropy, sprt::Sprt, wilson};
use electro_distill::store::EigenTuneStore;
use electro_distill::types::QualitySignal;

/// Real multi-turn conversations that a ELECTRO user would have.
...
async fn proof_04_graduation_pipeline() {
    ...
    loop {
        ...
        match decision {
            electro_distill::stats::sprt::SprtDecision::AcceptH1 => {
                println!(
            ...
            electro_distill::stats::sprt::SprtDecision::AcceptH0 => {
                println!(
...
async fn proof_05_status_report() {
    ...
    let engine = electro_distill::EigenTuneEngine::new(&config, "sqlite::memory:")
        .await
        .unwrap();
