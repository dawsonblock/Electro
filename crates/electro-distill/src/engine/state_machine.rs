...
impl EigenTuneStateMachine {
    ...
    pub async fn check_transition(
        &self,
        tier: EigenTier,
    ) -> Result<Option<TierState>, electro_core::types::error::ElectroError> {
    ...
    pub async fn transition(
        ...
        if record.state != from {
            return Err(electro_core::types::error::ElectroError::Internal(format!(
    ...
    pub async fn state(
        &self,
        tier: EigenTier,
    ) -> Result<TierState, electro_core::types::error::ElectroError> {
        let record = self.store.get_tier(tier.as_str()).await?;
        Ok(record.state)
    }
}
