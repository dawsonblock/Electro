...
/// Directory under `~/.electro/` where browser session cookies are stored.
const SESSIONS_DIR: &str = "sessions";

/// Shared allowlist for browser and web-fetch public destinations.
const PUBLIC_WEB_ALLOWLIST_ENV: &str = "ELECTRO_PUBLIC_WEB_ALLOWLIST";
...
impl BrowserTool {
    ...
    async fn ensure_browser(&self) -> Result<Page, ElectroError> {
        ...
        ensure_browser_runtime_policy("browser tool")?;

        // ── Browser mode: prefer remote isolated browser by default ──
        // Local launch is still present as an explicit escape hatch, but the
        // default upgraded runtime expects an isolated containerized browser
        // reachable over CDP on localhost.
        let remote_browser = browser_uses_remote();
        let force_headless = std::env::var("ELECTRO_HEADLESS").unwrap_or_default() == "1";
        let has_display = std::env::var("DISPLAY").is_ok()
            || std::env::var("WAYLAND_DISPLAY").is_ok()
            || cfg!(target_os = "macos")
            || cfg!(target_os = "windows");
        let use_headless = remote_browser || force_headless || !has_display;

        let mut builder = BrowserConfig::builder();

        if use_headless {
            builder = builder.arg("--headless=new");
            tracing::info!(mode = %browser_isolation_mode(), "Browser launching in headless mode");
        } else {
            builder = builder
                .arg("--window-position=0,0")
                .arg("--window-size=1280,900");
            tracing::info!(mode = %browser_isolation_mode(), "Browser launching in headed mode (local only)");
        }
        ...
        builder = builder
            ...
            .arg("--disable-features=AutofillServerCommunication,CertificateTransparencyComponentUpdater,DialMediaRouteProvider,MediaRouter");
        let proxy = std::env::var("ELECTRO_BROWSER_PROXY_SERVER").unwrap_or_default();
        if !remote_browser && !proxy.trim().is_empty() {
            tracing::info!(proxy = %proxy, "Browser proxy enabled");
            builder = builder.arg(format!("--proxy-server={}", proxy.trim()));
            let bypass = std::env::var("ELECTRO_BROWSER_PROXY_BYPASS").unwrap_or_default();
            if !bypass.trim().is_empty() {
                builder = builder.arg(format!("--proxy-bypass-list={}", bypass.trim()));
            }
        }
                ...
                if !default_subdir.join("Cookies").exists() {
                    ...
                    if let Some(ref real) = real_profile {
                        ...
                        tracing::warn!(
                            ...
                            "Browser: inherited live Chrome profile because ELECTRO_INHERIT_BROWSER_SESSION=1"
                        );
                    } else {
                        tracing::info!("Browser: no Chrome profile found, using fresh profile");
                    }
                } else {
                    tracing::warn!(profile = %work_profile.display(), "Browser: reusing inherited work profile because ELECTRO_INHERIT_BROWSER_SESSION=1");
                }
            }

            builder = builder.user_data_dir(&work_profile).arg("--no-first-run");
        } else {
            tracing::info!(mode = %browser_isolation_mode(), "Browser will connect to remote isolated runtime; local profile handling skipped");
        }

        // ELECTRO_NO_STEALTH=1 disables all anti-detection flags (for sites
        // like Zalo that detect stealth flags themselves and show blank pages)
        let no_stealth = std::env::var("ELECTRO_NO_STEALTH").unwrap_or_default() == "1";
        ...
        if !no_stealth {
            ...
            tracing::info!("Stealth flags DISABLED (ELECTRO_NO_STEALTH=1)");
        }

        let config = builder
            .window_size(1920, 1080)
            .build()
            .map_err(|e| ElectroError::Tool(format!("Failed to build browser config: {}", e)))?;

        let (mut browser, mut handler) = connect_or_launch_browser(config, "browser tool").await?;
        ...
        *self.cdp_handle.lock().await = Some(cdp_handle);
        ...
        let page = browser
            ...
            .await
            .map_err(|e| ElectroError::Tool(format!("Failed to create page: {}", e)))?;
        ...
        page.execute(AddScriptToEvaluateOnNewDocumentParams::new(STEALTH_JS))
            .await
            .map_err(|e| ElectroError::Tool(format!("Failed to inject stealth patches: {}", e)))?;
        ...
        *page_guard = Some(page.clone());
        ...
        tracing::info!(
            ...
            mode = %browser_isolation_mode(),
            "Browser ready"
        );
        Ok(page)
    }

    /// Save all browser cookies to a session file under `~/.electro/sessions/`.
    async fn save_session(&self, page: &Page, session_name: &str) -> Result<String, ElectroError> {
        // Get all cookies via CDP (Network.getCookies with no URL filter = all cookies)
        let response = page
            .execute(GetCookiesParams::default())
            .await
            .map_err(|e| ElectroError::Tool(format!("Failed to get cookies via CDP: {}", e)))?;
        ...
        let json = serde_json::to_string_pretty(&cookies).map_err(|e| {
            ElectroError::Tool(format!("Failed to serialize session cookies: {}", e))
        })?;

        // Write to ~/.electro/sessions/{name}.json
        let sessions_dir = sessions_dir()?;
        std::fs::create_dir_all(&sessions_dir).map_err(|e| {
            ElectroError::Tool(format!("Failed to create sessions directory: {}", e))
        })?;
        ...
        std::fs::write(&path, &json)
            .map_err(|e| ElectroError::Tool(format!("Failed to write session file: {}", e)))?;
    ...
    /// Restore browser cookies from a session file under `~/.electro/sessions/`.
    async fn restore_session(
        ...
        let json = std::fs::read_to_string(&path)
            .map_err(|e| ElectroError::Tool(format!("Failed to read session file: {}", e)))?;

        let cookies: Vec<SessionCookie> = serde_json::from_str(&json)
            .map_err(|e| ElectroError::Tool(format!("Failed to parse session file: {}", e)))?;
        ...
        page.execute(SetCookiesParams::new(cookie_params))
            .await
            .map_err(|e| ElectroError::Tool(format!("Failed to set cookies via CDP: {}", e)))?;
...
async fn cdp_insert_text(page: &Page, text: &str) -> Result<(), ElectroError> {
    use chromiumoxide::cdp::browser_protocol::input::InsertTextParams;
    page.execute(InsertTextParams::new(text))
        .await
        .map_err(|e| ElectroError::Tool(format!("insertText failed: {}", e)))?;
    Ok(())
}
...
async fn cdp_focus_backend_node(
    ...
    .await
    .map_err(|e| ElectroError::Tool(format!("DOM.focus failed: {}", e)))?;
    Ok(())
}
...
async fn cdp_clear_field(
    ...
    let resolved = page
        ...
        .await
        .map_err(|e| ElectroError::Tool(format!("DOM.resolveNode failed: {}", e)))?;
    let object_id = resolved
        .result
        .object
        .object_id
        .ok_or_else(|| ElectroError::Tool("Resolved node has no remote object ID".into()))?;
    ...
    page.execute(call_params)
        .await
        .map_err(|e| ElectroError::Tool(format!("callFunctionOn (clear) failed: {}", e)))?;
    Ok(())
}
...
async fn cdp_click_backend_node(
    ...
    page.execute(call_params)
        .await
        .map_err(|e| ElectroError::Tool(format!("callFunctionOn (click) failed: {}", e)))?;
    Ok(())
}
...
impl Tool for BrowserTool {
    ...
    fn description(&self) -> &str {
        "Control a stealth Chrome browser to navigate websites, click elements, type text, \
         ...
         - evaluate: Execute JavaScript and return the result (disabled by default; requires ELECTRO_BROWSER_ALLOW_EVAL=1)\n\
    ...
    fn declarations(&self) -> CapabilityPolicy {
        CapabilityPolicy {
            file_access: vec![
                FileAccessPolicy::ReadWrite("~/.electro/sessions".into()),
                FileAccessPolicy::Write(".".into()),
            ],
            network_access: electro_core::net_policy::NetworkPolicy::PublicWeb { allowlist: None },
            shell_access: electro_core::policy::ShellPolicy::Blocked,
            browser_access: electro_core::policy::BrowserPolicy::Allowed {
    ...
    async fn execute(
        ...
        let action = input
            ...
            .get("action")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ElectroError::Tool("Missing required parameter: action".into()))?;
        ...
        match action {
            "navigate" => {
                let url = input
                    ...
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| {
                        ElectroError::Tool("'navigate' requires 'url' parameter".into())
                    })?;

                let parsed = validate_public_url(url)
                    .map_err(ElectroError::Tool)?;
                ensure_resolved_host_is_public(&parsed)
                    .await
                    .map_err(ElectroError::Tool)?;
                let allowlist = browser_domain_allowlist();
                let host = parsed
                    .host_str()
                    .ok_or_else(|| ElectroError::Tool("Navigate URL must include a host".into()))?;
                enforce_host_allowlist(host, &allowlist, "browser navigation host")
                    .map_err(ElectroError::Tool)?;
                ...
                let title = page
                    .get_title()
                    .await
                    .map_err(|e| ElectroError::Tool(format!("Failed to get title: {}", e)))?
                    .unwrap_or_default();

                let current_url = page
                    .url()
                    .await
                    .map_err(|e| ElectroError::Tool(format!("Failed to get URL: {}", e)))?
                    .map(|u| u.to_string())
                    .unwrap_or_default();

                if !current_url.is_empty() {
                    let final_url = validate_public_url(&current_url)
                        .map_err(|msg| ElectroError::Tool(format!("Browser ended on blocked URL after navigation: {}", msg)))?;
                    ensure_resolved_host_is_public(&final_url)
                        .await
                        .map_err(|msg| ElectroError::Tool(format!("Browser final URL resolution blocked: {}", msg)))?;
                    if let Some(final_host) = final_url.host_str() {
                        enforce_host_allowlist(final_host, &allowlist, "browser final navigation host")
                            .map_err(ElectroError::Tool)?;
                    }
                }
            ...
            "click" => {
                let selector = input
                    ...
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| {
                        ElectroError::Tool("'click' requires 'selector' parameter".into())
                    })?;
                ...
                element
                    .click()
                    .await
                    .map_err(|e| ElectroError::Tool(format!("Click failed: {}", e)))?;
            ...
            "click_at" => {
                let x = input
                    ...
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| {
                        ElectroError::Tool("'click_at' requires 'x' parameter".into())
                    })?;
                let y = input
                    .arguments
                    .get("y")
                    .and_then(|v| v.as_f64())
                    .ok_or_else(|| {
                        ElectroError::Tool("'click_at' requires 'y' parameter".into())
                    })?;
                ...
                page.execute(move_cmd)
                    .await
                    .map_err(|e| ElectroError::Tool(format!("Mouse move failed: {}", e)))?;
                ...
                page.execute(down_cmd)
                    .await
                    .map_err(|e| ElectroError::Tool(format!("Mouse down failed: {}", e)))?;
                ...
                page.execute(up_cmd)
                    .await
                    .map_err(|e| ElectroError::Tool(format!("Mouse up failed: {}", e)))?;
                ...
                if let Some(text) = input.arguments.get("text").and_then(|v| v.as_str()) {
                    ...
                    .await
                    .map_err(|e| ElectroError::Tool(format!("Type after click failed: {}", e)))?;
                }
            ...
            "type" => {
                let selector = input
                    ...
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| {
                        ElectroError::Tool("'type' requires 'selector' parameter".into())
                    })?;
                let text = input
                    .arguments
                    .get("text")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| ElectroError::Tool("'type' requires 'text' parameter".into()))?;
                ...
                element
                    .click()
                    .await
                    .map_err(|e| ElectroError::Tool(format!("Failed to focus element: {}", e)))?;

                element
                    .type_str(text)
                    .await
                    .map_err(|e| ElectroError::Tool(format!("Type failed: {}", e)))?;
            ...
            "screenshot" => {
                ...
                let png_data = page
                    ...
                    .await
                    .map_err(|e| ElectroError::Tool(format!("Screenshot failed: {}", e)))?;

                tokio::fs::write(&save_path, &png_data)
                    .await
                    .map_err(|e| ElectroError::Tool(format!("Failed to save screenshot: {}", e)))?;
            ...
            "get_text" => {
                ...
                let text: String = page
                    ...
                    .await
                    .map_err(|e| ElectroError::Tool(format!("Failed to get text: {}", e)))?
                    .into_value()
                    .map_err(|e| ElectroError::Tool(format!("Failed to parse text: {:?}", e)))?;
            ...
            "evaluate" => {
                ...
                let script = input
                    ...
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| {
                        ElectroError::Tool("'evaluate' requires 'script' parameter".into())
                    })?;
                ...
                let result: serde_json::Value = page
                    .evaluate(script)
                    .await
                    .map_err(|e| ElectroError::Tool(format!("JS evaluation failed: {}", e)))?
                    .into_value()
                    .map_err(|e| {
                        ElectroError::Tool(format!("Failed to parse JS result: {:?}", e))
                    })?;
            ...
            "get_html" => {
                ...
                let html: String = if let Some(sel) = selector {
                    ...
                    page.evaluate(script)
                        .await
                        .map_err(|e| ElectroError::Tool(format!("Failed to get HTML: {}", e)))?
                        .into_value()
                        .map_err(|e| ElectroError::Tool(format!("Failed to parse HTML: {:?}", e)))?
                } else {
            ...
            "accessibility_tree" | "observe_tree" => {
                ...
                let result = page
                    ...
                    .await
                    .map_err(|e| {
                        ElectroError::Tool(format!("Accessibility tree extraction failed: {}", e))
                    })?;
            ...
            "observe" => {
                ...
                let tree_text = page
                    .evaluate(ax_js)
                    .await
                    .map_err(|e| ElectroError::Tool(format!("Observe: tree extraction failed: {}", e)))?
                    .into_value::<String>()
            ...
            "authenticate" => {
                let service = input.arguments.get("service").and_then(|v| v.as_str())
                    .ok_or_else(|| ElectroError::Tool("'authenticate' requires 'service' parameter".into()))?;

                tracing::info!(service = %service, "Browser authenticate — credential isolation protocol");

                let vault = self.vault.as_ref()
                    .ok_or_else(|| ElectroError::Tool("Vault not available".into()))?;
                let raw_bytes = vault.get_secret(&format!("web_cred:{}", service)).await?
                    .ok_or_else(|| ElectroError::Tool(format!("No credentials for '{}'", service)))?;
                ...
                let ax_result = page.execute(GetFullAxTreeParams::default()).await
                    .map_err(|e| ElectroError::Tool(format!("Auth: ax tree failed: {}", e)))?;

                let (user_id, pass_id, submit_id) = detect_login_form(&ax_result.result.nodes)
                    .ok_or_else(|| ElectroError::Tool("Could not detect login form on this page".into()))?;
                ...
                let user_node = find_ax_node_by_id(&ax_result.result.nodes, &user_id)
                    .ok_or_else(|| ElectroError::Tool("Username AX node not found".into()))?;
                let pass_node = find_ax_node_by_id(&ax_result.result.nodes, &pass_id)
                    .ok_or_else(|| ElectroError::Tool("Password AX node not found".into()))?;
                let submit_node = find_ax_node_by_id(&ax_result.result.nodes, &submit_id)
                    .ok_or_else(|| ElectroError::Tool("Submit AX node not found".into()))?;

                let user_backend_id = user_node.backend_dom_node_id
                    .ok_or_else(|| ElectroError::Tool("Username AX node has no DOM backing".into()))?;
                let pass_backend_id = pass_node.backend_dom_node_id
                    .ok_or_else(|| ElectroError::Tool("Password AX node has no DOM backing".into()))?;
                let submit_backend_id = submit_node.backend_dom_node_id
                    .ok_or_else(|| ElectroError::Tool("Submit AX node has no DOM backing".into()))?;
                ...
                let post_ax = page.execute(GetFullAxTreeParams::default()).await
                    .map_err(|e| ElectroError::Tool(format!("Auth: post-login tree failed: {}", e)))?;
            ...
            "restore_web_session" => {
                let service = input.arguments.get("service").and_then(|v| v.as_str())
                    .ok_or_else(|| ElectroError::Tool("'restore_web_session' requires 'service' parameter".into()))?;

                tracing::info!(service = %service, "Browser restore_web_session — loading session from vault");

                let vault = self.vault.as_ref()
                    .ok_or_else(|| ElectroError::Tool("Vault not available for session restore".into()))?;
...
mod tests {
    ...
    fn sessions_dir_returns_correct_path() {
        ...
        assert!(
            path_str.ends_with(".electro/sessions"),
            "Sessions dir should end with .electro/sessions, got: {}",
            path_str
        );
    }
