//! ELECTRO Tools — agent capabilities (shell, file, web, browser, etc.)
...
pub async fn browser_session_login(
    service: &str,
    url: &str,
    vault: &dyn electro_core::Vault,
) -> Result<String, electro_core::types::error::ElectroError> {
    ...
    loop {
        ...
        .await
        .map_err(|e| {
            electro_core::types::error::ElectroError::Tool(format!("Failed to read input: {}", e))
        })?;
