...
impl InteractiveBrowseSession {
    ...
    pub async fn new(browser: &Browser, service: &str, url: &str) -> Result<Self, ElectroError> {
        let parsed = validate_public_url(url).map_err(ElectroError::Tool)?;
        ...
        if let Some(host) = parsed.host_str() {
            enforce_host_allowlist(host, &allowlist, "interactive browser session host")
                .map_err(ElectroError::Tool)?;
        }
        ...
        let page = browser
            .new_page(url)
            .await
            .map_err(|e| ElectroError::Tool(format!("Failed to create page for session: {}", e)))?;
        ...
        let current_url = page
            .url()
            .await
            .map_err(|e| ElectroError::Tool(format!("Failed to get session URL: {}", e)))?
            .map(|u| u.to_string())
            .unwrap_or_default();
    ...
    pub async fn launch(service: &str, url: &str) -> Result<Self, ElectroError> {
        use chromiumoxide::browser::BrowserConfig;
        use futures::StreamExt;

        ensure_browser_runtime_policy("interactive browser session")?;
        ...
        if !remote_browser {
            ...
            if inherit_browser_session && !force_clean_browser && work_profile.exists() {
                builder = builder.user_data_dir(&work_profile).arg("--no-first-run");
            } else {
                let tp = std::env::temp_dir().join(format!("electro-login-{}", std::process::id()));
                let _ = std::fs::remove_dir_all(&tp);
                let _ = std::fs::create_dir_all(&tp);
                builder = builder.user_data_dir(&tp).arg("--incognito");
            }
        } else {
            tracing::info!(mode = %browser_isolation_mode(), "Interactive session will use remote isolated browser");
        }
    ...
    pub async fn capture_annotated(&mut self) -> Result<(Vec<u8>, String), ElectroError> {
        ...
        let js_result = self
            ...
            .evaluate(js)
            .await
            .map_err(|e| ElectroError::Tool(format!("Session element scan failed: {}", e)))?;
        ...
        let png_data = self
            ...
            .await
            .map_err(|e| ElectroError::Tool(format!("Session screenshot failed: {}", e)))?;
    ...
    pub async fn handle_input(&mut self, input: &str) -> Result<SessionAction, ElectroError> {
        ...
        if let Some(num) = num_part {
            if self.click_coords.contains_key(&num) {
                ...
                let result = self
                    ...
                    .evaluate(click_js)
                    .await
                    .map_err(|e| ElectroError::Tool(format!("Click element: {}", e)))?;
        ...
        let focus_check: String = self
            ...
            .await
            .map_err(|e| ElectroError::Tool(format!("Focus check failed: {}", e)))?
            .into_value()
        ...
        if focus_check == "has_focus" {
            ...
            self.page
                ...
                .await
                .map_err(|e| ElectroError::Tool(format!("Text insertion failed: {}", e)))?;
    ...
    pub async fn capture_session(&self, vault: &dyn Vault) -> Result<(), ElectroError> {
        ...
        let cookies_response = self
            ...
            .await
            .map_err(|e| ElectroError::Tool(format!("Session: get cookies failed: {}", e)))?;
...
pub async fn restore_web_session(
    ...
    vault: &dyn Vault,
    service: &str,
) -> Result<(String, bool), ElectroError> {
    let vault_key = format!("web_session:{}", service);
    let raw_bytes = vault
        .get_secret(&vault_key)
        .await?
        .ok_or_else(|| ElectroError::Tool(format!("No saved session for '{}'", service)))?;

    let state: SessionState = serde_json::from_slice(&raw_bytes)
        .map_err(|e| ElectroError::Tool(format!("Session state parse error: {}", e)))?;
    ...
    if !cookie_params.is_empty() {
        page.execute(SetCookiesParams::new(cookie_params))
            .await
            .map_err(|e| {
                ElectroError::Tool(format!("Session restore: set cookies failed: {}", e))
            })?;
    }
    ...
    page.goto(&state.url)
        .await
        .map_err(|e| ElectroError::Tool(format!("Session restore: navigation failed: {}", e)))?;
    ...
    let verify_result = page
        .evaluate(verify_js)
        .await
        .map_err(|e| ElectroError::Tool(format!("Session verify failed: {}", e)))?;
