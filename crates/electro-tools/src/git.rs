...
impl GitTool {
        ...
        match action {
            "clone" => {
                let url = args
                    .get("url")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| ElectroError::Tool("clone requires 'url' parameter".into()))?;
                cmd_args.push(url.to_string());

                if let Some(dir_str) = args.get("directory").and_then(|v| v.as_str()) {
                    // Use a dummy root here as we'll re-validate in execute, or just validate safety
                    // Actually, build_args doesn't have ctx. Let's do validation in execute.
                    cmd_args.push(dir_str.to_string());
                }
            }
            "pull" => {
            ...
            "commit" => {
                let message = args
                    ...
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| {
                        ElectroError::Tool("commit requires 'message' parameter".into())
                    })?;
            ...
            "checkout" => {
                let branch = args.get("branch").and_then(|v| v.as_str()).ok_or_else(|| {
                    ElectroError::Tool("checkout requires 'branch' parameter".into())
                })?;
            ...
            "add" => {
                ...
                if all {
                    ...
                    let files = args
                        ...
                        .ok_or_else(|| {
                            ElectroError::Tool("add requires 'files' array or 'all: true'".into())
                        })?;
    ...
    fn validate_safety(action: &str, args: &serde_json::Value) -> Result<(), ElectroError> {
