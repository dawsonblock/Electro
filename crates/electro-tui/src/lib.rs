//! ELECTRO Interactive TUI — Claude Code-level terminal experience.
//!
//! Launch via `electro tui`. Provides:
...
///
/// This is the main entry point called from `electro tui`.
pub async fn launch_tui(config: ElectroConfig) -> anyhow::Result<()> {
    ...
    if agent_handle.is_some() {
        ...
        let welcome = format!(
            "Welcome to ELECTRO TUI! Provider: {} | Model: {}\n\
    ...
    loop {
        ...
        if let Some(text) = state.pending_user_message.take() {
            if let Some(ref handle) = agent_handle {
                ...
                let msg = electro_core::types::message::InboundMessage {
...
fn parse_file_references(text: &str) -> (String, Vec<electro_core::types::message::AttachmentRef>) {
    ...
    if let Some(path_str) = trimmed.strip_prefix("/file ") {
        ...
        if expanded.exists() {
            let att = electro_core::types::message::AttachmentRef {
