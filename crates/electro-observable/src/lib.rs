...
use electro_core::types::config::ObservabilityConfig;
use electro_core::types::error::ElectroError;

// ── Predefined metric names ────────────────────────────────────────────

/// Provider call latency in milliseconds.
pub const METRIC_PROVIDER_LATENCY: &str = "electro.provider.latency_ms";

/// Total number of tool executions.
pub const METRIC_TOOL_EXECUTIONS: &str = "electro.tool.executions";

/// Total number of tool execution errors.
pub const METRIC_TOOL_ERRORS: &str = "electro.tool.errors";

/// Total tokens consumed across all providers.
pub const METRIC_TOKENS_USED: &str = "electro.tokens.used";

/// Tasks (agent loops) that ran to completion.
pub const METRIC_TASK_COMPLETIONS: &str = "electro.task.completions";

/// Memory backend operations (store, search, delete, …).
pub const METRIC_MEMORY_OPS: &str = "electro.memory.operations";
...
mod tests {
    ...
    fn predefined_metric_names() {
        assert_eq!(METRIC_PROVIDER_LATENCY, "electro.provider.latency_ms");
        assert_eq!(METRIC_TOOL_EXECUTIONS, "electro.tool.executions");
        assert_eq!(METRIC_TOOL_ERRORS, "electro.tool.errors");
        assert_eq!(METRIC_TOKENS_USED, "electro.tokens.used");
        assert_eq!(METRIC_TASK_COMPLETIONS, "electro.task.completions");
        assert_eq!(METRIC_MEMORY_OPS, "electro.memory.operations");
    }
    ...
    async fn factory_produced_observable_is_functional() {
        ...
        assert!(matches!(
            health.status,
            electro_core::traits::HealthState::Healthy
        ));
    }
}
