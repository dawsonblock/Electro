...
use electro_core::traits::{ComponentHealth, HealthState, HealthStatus, Observable};
...
impl OtelExporter {
    ...
    pub fn new(endpoint: &str) -> Result<Self, ElectroError> {
...
impl Observable for OtelExporter {
    ...
    async fn health_status(&self) -> Result<HealthStatus, ElectroError> {
        // This exporter records metrics in-process only. Without a real
        // OTLP transport implementation we should not report full health.
        Ok(HealthStatus {
