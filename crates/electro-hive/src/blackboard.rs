...
impl Blackboard {
    ...
    pub async fn new(database_url: &str) -> Result<Self, ElectroError> {
        let pool = SqlitePoolOptions::new()
            ...
            .await
            .map_err(|e| ElectroError::Internal(format!("Blackboard connect: {e}")))?;
    ...
    async fn init_tables(&self) -> Result<(), ElectroError> {
        sqlx::query(
            r#"
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("Blackboard orders table: {e}")))?;
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("Blackboard tasks table: {e}")))?;
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("Blackboard index: {e}")))?;
    ...
    pub async fn create_order(&self, order: &HiveOrder) -> Result<(), ElectroError> {
        sqlx::query(
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("Blackboard create_order: {e}")))?;
    ...
    /// Get an order by ID.
    pub async fn get_order(&self, order_id: &str) -> Result<Option<HiveOrder>, ElectroError> {
        ...
        .await
        .map_err(|e| ElectroError::Internal(format!("Blackboard get_order: {e}")))?;
    ...
    pub async fn create_tasks(&self, tasks: &[HiveTask]) -> Result<(), ElectroError> {
        for task in tasks {
            ...
            let deps_json = serde_json::to_string(&task.dependencies)
                .map_err(|e| ElectroError::Internal(format!("serialize deps: {e}")))?;
            let tags_json = serde_json::to_string(&task.context_tags)
                .map_err(|e| ElectroError::Internal(format!("serialize tags: {e}")))?;
            ...
            .execute(&self.pool)
            .await
            .map_err(|e| ElectroError::Internal(format!("Blackboard create_task: {e}")))?;
        }
    ...
    pub async fn claim_task(&self, task_id: &str, worker_id: &str) -> Result<bool, ElectroError> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("Blackboard claim_task: {e}")))?;
    ...
    pub async fn complete_task(
        ...
        result_summary: &str,
        actual_tokens: u32,
    ) -> Result<Vec<String>, ElectroError> {
        let now = chrono::Utc::now().timestamp_millis();
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("Blackboard complete_task: {e}")))?;
        ...
        let (order_id,): (String,) =
            sqlx::query_as("SELECT order_id FROM hive_tasks WHERE id = ?1")
                ...
                .fetch_one(&self.pool)
                .await
                .map_err(|e| ElectroError::Internal(format!("Blackboard get order_id: {e}")))?;
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("Blackboard update order: {e}")))?;
    ...
    pub async fn fail_task(
        ...
        task_id: &str,
        error: &str,
    ) -> Result<HiveTaskStatus, ElectroError> {
        ...
        let row: (i64, i64) =
            sqlx::query_as("SELECT retry_count, max_retries FROM hive_tasks WHERE id = ?1")
                ...
                .fetch_one(&self.pool)
                .await
                .map_err(|e| ElectroError::Internal(format!("Blackboard fail_task fetch: {e}")))?;
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("Blackboard fail_task: {e}")))?;
    ...
    pub async fn get_ready_tasks(&self, order_id: &str) -> Result<Vec<HiveTask>, ElectroError> {
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("Blackboard get_ready: {e}")))?;

        rows.into_iter().map(|r| r.into_task()).collect()
    }

    /// Get a task by ID.
    pub async fn get_task(&self, task_id: &str) -> Result<Option<HiveTask>, ElectroError> {
        ...
        .fetch_optional(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("Blackboard get_task: {e}")))?;
    ...
    pub async fn get_dependency_results(
        &self,
        task_id: &str,
    ) -> Result<Vec<(String, String)>, ElectroError> {
        // First get the task's dependencies
        let task = self
            .get_task(task_id)
            .await?
            .ok_or_else(|| ElectroError::Internal(format!("Task not found: {task_id}")))?;
        ...
        for dep_id in &task.dependencies {
            let dep = self
                .get_task(dep_id)
                .await?
                .ok_or_else(|| ElectroError::Internal(format!("Dependency not found: {dep_id}")))?;
    ...
    pub async fn is_order_complete(&self, order_id: &str) -> Result<bool, ElectroError> {
        ...
        .fetch_one(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("Blackboard is_complete: {e}")))?;

        Ok(non_terminal == 0)
    }

    /// Get all tasks for an order in topological order (completed tasks first).
    pub async fn get_order_results(&self, order_id: &str) -> Result<Vec<HiveTask>, ElectroError> {
        let rows: Vec<TaskRow> = sqlx::query_as(
            "SELECT id, order_id, description, status, claimed_by, dependencies, \
             context_tags, estimated_tokens, actual_tokens, result_summary, artifacts, \
             retry_count, max_retries, error_log, created_at, started_at, completed_at \
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("Blackboard get_results: {e}")))?;
    ...
    async fn resolve_dependencies(&self, order_id: &str) -> Result<Vec<String>, ElectroError> {
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("Blackboard resolve: {e}")))?;
        ...
        for row in pending {
            ...
            for dep_id in &task.dependencies {
                let (dep_status,): (String,) =
                    sqlx::query_as("SELECT status FROM hive_tasks WHERE id = ?1")
                        ...
                        .await
                        .map_err(|e| {
                            ElectroError::Internal(format!("Blackboard resolve dep check: {e}"))
                        })?;
            ...
            if all_deps_met {
                sqlx::query("UPDATE hive_tasks SET status = 'ready' WHERE id = ?1")
                    ...
                    .await
                    .map_err(|e| {
                        ElectroError::Internal(format!("Blackboard resolve update: {e}"))
                    })?;
...
impl TaskRow {
    fn into_task(self) -> Result<HiveTask, ElectroError> {
        ...
        Ok(HiveTask {
            ...
            status: HiveTaskStatus::parse_str(&self.status).ok_or_else(|| {
                ElectroError::Internal(format!("Unknown task status: {}", self.status))
            })?,
