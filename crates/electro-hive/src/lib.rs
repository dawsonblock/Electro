...
//!
//! The Hive coordinates multiple ELECTRO agent workers to execute complex
...
impl Hive {
    ...
    where
        F: Fn(String) -> Fut,
        Fut: std::future::Future<Output = Result<(String, u64), ElectroError>>,
    {
    ...
    where
        ...
        Fut: std::future::Future<Output = Result<TaskResult, ElectroError>> + Send,
    {
        ...
        let order = self
            ...
            .get_order(order_id)
            .await?
            .ok_or_else(|| ElectroError::Internal(format!("Order not found: {order_id}")))?;
