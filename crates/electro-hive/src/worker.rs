...
impl HiveWorker {
    ...
    pub async fn run_loop<F, Fut>(
        ...
        execute_fn: F,
    ) -> Result<WorkerStats, ElectroError>
    where
        F: Fn(HiveTask, Vec<(String, String)>) -> Fut,
        Fut: std::future::Future<Output = Result<TaskResult, ElectroError>>,
    {
        ...
        loop {
            ...
            let task = blackboard.get_task(&task_id).await?.ok_or_else(|| {
                ElectroError::Internal(format!("Claimed task vanished: {task_id}"))
            })?;
            ...
            let result = tokio::select! {
                ...
                _ = tokio::time::sleep(timeout) => {
                    Err(ElectroError::Internal(format!(
