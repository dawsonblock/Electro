...
impl PheromoneField {
    ...
    pub async fn new(database_url: &str, config: PheromoneConfig) -> Result<Self, ElectroError> {
        let pool = SqlitePoolOptions::new()
            .max_connections(5)
            .connect(database_url)
            .await
            .map_err(|e| ElectroError::Internal(format!("PheromoneField connect: {e}")))?;
    ...
    async fn init_tables(&self) -> Result<(), ElectroError> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("PheromoneField init: {e}")))?;
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("PheromoneField index: {e}")))?;
    ...
    pub async fn emit(
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("PheromoneField emit: {e}")))?;
    ...
    pub async fn read_total_at(
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("PheromoneField read: {e}")))?;
    ...
    pub async fn read_all_for_target(
        &self,
        target: &str,
    ) -> Result<HashMap<SignalType, f64>, ElectroError> {
        let now_ms = chrono::Utc::now().timestamp_millis();
    ...
    pub async fn gc(&self) -> Result<usize, ElectroError> {
        let now_ms = chrono::Utc::now().timestamp_millis();
        self.gc_at(now_ms).await
    }

    /// GC at a specific timestamp (for testing).
    pub async fn gc_at(&self, now_ms: i64) -> Result<usize, ElectroError> {
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("PheromoneField gc fetch: {e}")))?;
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| ElectroError::Internal(format!("PheromoneField gc urgency: {e}")))?;
    ...
    pub async fn signal_count(&self) -> Result<usize, ElectroError> {
        let (count,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM hive_pheromones")
            .fetch_one(&self.pool)
            .await
            .map_err(|e| ElectroError::Internal(format!("PheromoneField count: {e}")))?;
