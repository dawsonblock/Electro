...
//!
//! Run: GEMINI_API_KEY=... cargo test -p electro-hive --test project_bench_v2 -- --nocapture
...
const BUILD_DIR: &str = "/tmp/electro_bench_v2";
...
fn make_provider() -> Result<Arc<dyn Provider>, ElectroError> {
    ...
    let p = electro_providers::create_provider(&config)
        .map_err(|e| ElectroError::Provider(format!("{e}")))?;
    Ok(Arc::from(p))
}
...
async fn project_benchmark_v2() {
    ...
    println!("╔═══════════════════════════════════════════════════════╗");
    println!("║  ELECTRO HIVE — PROJECT BENCHMARK v2 (Difficulty 8)   ║");
