...
//!
//! Run: GEMINI_API_KEY=... cargo test -p electro-hive --test project_bench_py -- --nocapture
...
const BUILD_DIR: &str = "/tmp/electro_bench_py";
...
fn make_provider() -> Result<Arc<dyn Provider>, ElectroError> {
    ...
    Ok(Arc::from(
        electro_providers::create_provider(&config)
...
async fn python_project_benchmark() {
    ...
    println!("╔═══════════════════════════════════════════════════════╗");
    println!("║  ELECTRO HIVE — PYTHON PROJECT BENCHMARK (Diff 8)    ║");
