...
//!
//! Run: GEMINI_API_KEY=... cargo test -p electro-hive --test live_ab_bench -- --nocapture
...
fn make_provider() -> Result<Arc<dyn Provider>, ElectroError> {
    let key = std::env::var("GEMINI_API_KEY")
        .map_err(|_| ElectroError::Config("GEMINI_API_KEY not set".into()))?;
    ...
    let provider = electro_providers::create_provider(&config)
        .map_err(|e| ElectroError::Provider(format!("Failed to create provider: {e}")))?;
...
async fn single_call(
    ...
    prompt: &str,
) -> Result<(String, u32, u32, u64), ElectroError> {
...
async fn run_swarm(
    ...
    let config = electro_hive::HiveConfig {
        enabled: true,
        max_workers: 4,
        min_workers: 1,
    ...
    let hive = electro_hive::Hive::new(&config, "sqlite::memory:").await?;
    ...
    if let Some(ref oid) = order_id {
        ...
        let result = hive
            .execute_order(oid, cancel, move |task, deps| {
                ...
                async move {
                    let context = electro_hive::worker::build_scoped_context(&task, &deps);
                    let (text, inp, out, _) = single_call(&*p, &context).await?;
                    Ok(electro_hive::worker::TaskResult {
                        summary: text,
                        tokens_used: inp + out,
                        artifacts: vec![],
                        success: true,
...
async fn live_ab_benchmark() {
    ...
    // Verify connectivity
    println!("=== ELECTRO Hive A/B Benchmark (LIVE) ===");
...
async fn execution_time_benchmark() {
    ...
    let hive = electro_hive::Hive::new(&config, "sqlite::memory:")
        .await
        .expect("Hive init");

    // Manually create order + tasks (bypass Queen — we already know the decomposition)
    let now = chrono::Utc::now().timestamp_millis();
    let order = electro_hive::types::HiveOrder {
        id: "exec-bench".into(),
        chat_id: "bench".into(),
        original_message: "benchmark".into(),
        task_count: subtasks.len(),
        completed_count: 0,
        status: electro_hive::types::HiveOrderStatus::Active,
        total_tokens: 0,
        queen_tokens: 0,
        created_at: now,
        completed_at: None,
