...
This protocol defines how Claude Code (the agentic developer) must self-test and validate all ELECTRO work. The user does not have time or capacity to manually test — Claude must do it.
...
After compilation and unit tests pass, test the feature end-to-end by running ELECTRO directly.
...
# Start ELECTRO with CLI-accessible configuration
...
Always start ELECTRO with log output captured:
...
# Watch for errors
tail -f /tmp/electro.log | grep --line-buffered -E "ERROR|WARN|panic"

# Watch specific subsystem
tail -f /tmp/electro.log | grep --line-buffered "memory_manage"
```
