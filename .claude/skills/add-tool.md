# Skill: Add a new tool to ELECTRO

## When to use

Use this skill when the user asks to add a new tool capability for the ELECTRO agent (e.g., HTTP client, database query, image generation, code execution, web scraping).

## Reference implementation

Study the existing definitions:
- `crates/electro-core/src/traits/tool.rs` -- the `Tool` trait, `ToolDeclarations`, `ToolInput`, `ToolOutput`, `ToolContext`, `PathAccess`
- `crates/electro-tools/src/lib.rs` -- the tools crate (currently minimal, ready for implementations)
- `crates/electro-tools/Cargo.toml` -- tool crate dependencies and feature flags

## Steps

### 1. Create the tool source file

Create `crates/electro-tools/src/<tool_name>.rs` using the template below.

### 2. Add the module to lib.rs

Edit `crates/electro-tools/src/lib.rs`:
...
Edit `crates/electro-tools/Cargo.toml`:
...
- Add the feature flag: `<tool_name> = ["electro-tools/<tool_name>"]`
...
```bash
cargo check -p electro-tools
cargo test -p electro-tools
cargo clippy -p electro-tools -- -D warnings
```
...
use electro_core::{Tool, ToolDeclarations, ToolInput, ToolOutput, ToolContext, PathAccess};

/// <ToolName> tool for the ELECTRO agent.
...
impl Tool for <ToolName>Tool {
    ...
    async fn execute(
        ...
        //     .and_then(|v| v.as_str())
        //     .ok_or_else(|| ElectroError::Tool("Missing required parameter: some_param".into()))?;
...
- **Error vs. tool error**: Return `Err(ElectroError::Tool(...))` when the tool infrastructure fails (e.g., missing dependency). Return `Ok(ToolOutput { is_error: true, content: "..." })` when the tool ran but the operation failed (e.g., HTTP 404, file not found). The AI model sees `is_error: true` results and can retry or adjust.
