# Skill: Add a new memory backend to ELECTRO

## When to use

Use this skill when the user asks to add a new memory/storage backend (e.g., PostgreSQL, Redis, DynamoDB, Qdrant, ChromaDB, Pinecone) to ELECTRO.

## Reference implementation

Study the existing backends:
- `crates/electro-memory/src/sqlite.rs` -- full Memory trait implementation with SQLite via sqlx
- `crates/electro-memory/src/markdown.rs` -- file-based Memory implementation for OpenClaw compatibility
- `crates/electro-core/src/traits/memory.rs` -- the `Memory` trait definition plus `MemoryEntry`, `SearchOpts`, `MemoryEntryType`

## Steps

### 1. Create the backend source file

Create `crates/electro-memory/src/<backend_name>.rs` using the template below.

### 2. Add the module to lib.rs

Edit `crates/electro-memory/src/lib.rs`:
...
Edit `crates/electro-memory/Cargo.toml`:
...
- Add the feature flag: `<backend_name> = ["electro-memory/<backend_name>"]`
...
```bash
cargo check -p electro-memory
cargo test -p electro-memory
cargo clippy -p electro-memory -- -D warnings
```
...
use electro_core::{Memory, MemoryEntry, MemoryEntryType, SearchOpts};
...
impl <BackendName>Memory {
    ...
    pub async fn new(connection_url: &str) -> Result<Self, ElectroError> {
...
fn str_to_entry_type(s: &str) -> Result<MemoryEntryType, ElectroError> {
    match s {
        ...
        other => Err(ElectroError::Memory(format!(
...
- **Error types**: Use `ElectroError::Memory(...)` for all memory backend errors.
