...
This archive changes several defaults to fail closed. See `docs/HARDENING_NOTES.md`. The most important changes are: shell disabled by default, workspace-only file access, blocked local/private HTTP targets, clean browser profile by default, custom tools disabled by default, and no auto-bootstrap admin unless explicitly enabled.
